...
protect the forge dashboard. Each line is `username:bcrypt-hash` — passwords are **never**
stored in plaintext, so you can't edit them by hand. Use the `htpasswd` tool to
generate the hashed entries.

> The default entry here is a placeholder (`admin`, copied from the hunter stack).
> **Rotate it before exposing forge-live publicly** — it ships to EC2 with the same
> TLS + Basic-auth setup as hunter.
