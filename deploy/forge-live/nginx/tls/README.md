...
The `ui` (nginx) container terminates TLS using `cert.pem` + `key.pem` in this
...
openssl req -x509 -newkey rsa:2048 -nodes -days 365 \
  -keyout deploy/forge-live/nginx/tls/key.pem \
  -out    deploy/forge-live/nginx/tls/cert.pem \
  -subj "/CN=localhost" \
  -addext "subjectAltName=DNS:localhost,IP:127.0.0.1,IP:<host_ip_address>"
```
