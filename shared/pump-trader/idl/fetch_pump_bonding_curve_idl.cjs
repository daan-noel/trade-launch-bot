// One-shot: fetch the on-chain Anchor IDL for the pump.fun bonding-curve (pump) program
// and write it to pump_bonding_curve.json next to this script.
//
// Run with @solana/web3.js available on NODE_PATH, e.g. from the scratch dir:
//   NODE_PATH=/tmp/idlfetch/node_modules node fetch_pump_bonding_curve_idl.cjs
const fs = require("fs");
...
async function main() {
  const programId = new PublicKey(PUMP);
  ...
  const outPath = path.resolve(__dirname, "pump_bonding_curve.json");
