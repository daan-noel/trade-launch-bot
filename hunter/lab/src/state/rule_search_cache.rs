//! Disk-backed last-rule-search-result cache. Twin of [`super::discovery_result_cache`].
...
use crate::lake;
use crate::rule_search::report::Report;
...
pub struct RuleSearchCache {
    path: PathBuf,
    slot: RwLock<Option<(Uuid, Report)>>,
}

impl Default for RuleSearchCache {
    fn default() -> Self {
        Self::open(lake::rule_search_result_path(&lake::lake_root()))
    }
}

impl RuleSearchCache {
    pub fn new() -> Self {
        Self::default()
    }
        ...
        let slot = match load(&path) {
            ...
            Err(e) => {
                tracing::warn!(
                    path = %path.display(),
                    error = %e,
                    "rule-search: could not load cached result; starting empty"
                );
                None
            }
        };
    ...
    pub async fn store(&self, run_id: Uuid, result: Report) {
        ...
        if let Err(e) = write_atomic(&self.path, &stored) {
            tracing::warn!(
                ...
                error = %e,
                "rule-search: disk persist failed; result stays RAM-only"
            );
        }
