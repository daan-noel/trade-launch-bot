...
//!
//! Live-only concerns (lifecycle, real positions) are deploy's. The lab has no
//! runtime cache, so the rule list's position counters come from the **synced**
//! `strategy_positions` rows (latest paper run per rule), not live activity.
...
/// simulated. Matches the deploy edge's `rule_to_json`.
fn rule_to_json(
...
/// Read the [`LegacyRuleDraft`] inputs from the flat request body.
...
pub async fn simulate_tpsl_rule(
    ...
    crate::strategies::sim_spawn::spawn_rule_simulation(
        ...
        |state, rid, since, until, cancel, cell| {
            Box::pin(async move {
                let rows = crate::strategies::tpsl_sniper_2::run_backtest(
                    state, rid, since, until, cancel, cell,
                )
                .await?;
...
/// Start a backtest for every tpsl2 rule whose cached rollup is missing or stale
/// — the rules table's "Simulate All" action. See tpsl1's twin
/// ([`super::tpsl1::simulate_all_tpsl_rules`]) for the full rationale.
///
/// POST /api/strategies/tpsl2/rules/simulate-all
pub async fn simulate_all_tpsl_rules(
    ...
    let rules = match app_state.strategy_repo().find_rules_by_strategy(STRATEGY_ID).await {
        Ok(r) => r,
        Err(e) => {
            tracing::error!("Failed to list TPSL2 rules for simulate-all: {e}");
...
/// POST /api/strategies/tpsl2/rules/{rule_id}/positions/summary — filtered aggregates.
...
/// POST /api/strategies/tpsl2/rules/{rule_id}/simulate/result — one server-side
/// page of the finished backtest's per-token rows (in-memory, unified TableRequest).
pub async fn get_simulate_result_tpsl2(
    app_state: web::Data<Arc<LocalState>>,
    rule_id: web::Path<Uuid>,
    body: web::Json<TableRequest>,
) -> impl Responder {
...
/// POST /api/strategies/tpsl2/rules/{rule_id}/simulate/result/summary — filtered
/// aggregate over the finished backtest's rows for the Simulated summary card.
pub async fn get_simulate_result_summary_tpsl2(
    app_state: web::Data<Arc<LocalState>>,
    rule_id: web::Path<Uuid>,
    body: web::Json<TableRequest>,
) -> impl Responder {
