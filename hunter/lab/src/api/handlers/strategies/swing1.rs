...
/// Start a backtest for every swing1 rule whose cached rollup is missing or
/// stale — the rules table's "Simulate All" action. See tpsl1's twin
/// ([`super::tpsl1::simulate_all_tpsl_rules`]) for the full rationale.
///
/// POST /api/strategies/swing1/rules/simulate-all
pub async fn simulate_all_swing1_rules(
    app_state: web::Data<Arc<LocalState>>,
    range: web::Query<AnalysisRange>,
) -> impl Responder {
    let (since, until) = (range.from, range.to);
    let rules = match app_state.strategy_repo().find_rules_by_strategy(STRATEGY_ID).await {
        Ok(r) => r,
        Err(e) => {
            tracing::error!("Failed to list swing1 rules for simulate-all: {e}");
