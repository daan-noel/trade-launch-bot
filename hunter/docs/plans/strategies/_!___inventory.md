...
| **Firing structure silent** | The ix structure we fired on stops printing while we are under the fill | Sells. **Firing structure sells** waits for it to leave; this only waits for it to stop. | Its buying was the reason we bought, and a buyer that has gone quiet is no longer that reason. Never read at this grain: the booked cut is **Coin tape stops**, which waits for the whole coin to fall quiet, and it is the best state cut the exit lab finds - so the narrower reading is a live candidate, not a refuted one. | 1. We are under the fill.<br>2. No print from that structure for 20 slots.<br>3. We sell. | new |
