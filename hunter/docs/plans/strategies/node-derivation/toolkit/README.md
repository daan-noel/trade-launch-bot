...
| `trigger` | 5.1, 8.1 | `excess_intensity(S, {group: [ids]}, cases="buy"/"close", controls="coin"/"hold", near="node"/"group")`; `peak(lift, cls)` | lift and excess tables, class x lag bin (14 lag bins to 5 s). Print-class families (WHO and this-print history, not only size / % / gap): [derive section 5](../../_!___derive.md#print-classes-51-scan) |
