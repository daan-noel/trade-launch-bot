"""Rule 1 in the engine, ticket by ticket against the frozen reference (step 3 of
...
(slot, tx_index, leg). This script writes that test's inputs and reads its output.
  ...
  python r1_engine_parity.py compare TAPE ENGINE_CSV
      matches the engine rows to r1_ref_TAPE.parquet on the trigger print and reports every
      difference in the entry fill, the exit print, the reason and the SOL; then the two books
...
def engine_frame(T: Tape, eng: pd.DataFrame) -> pd.DataFrame:
    ...
    return pd.DataFrame({"y": eng.pnl_sol.to_numpy(), "run": eng.mint.map(code).to_numpy(),
                         "why": eng.reason.map(lambda s: REASON.get(s, "time")).to_numpy(),
...
def compare(name: str, csv: str) -> None:
    ...
    both["why_e"] = both.reason.map(lambda s: REASON.get(s, "time" if str(s).startswith("held") else s))
    checks = {
