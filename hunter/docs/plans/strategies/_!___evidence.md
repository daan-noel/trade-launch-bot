...
[_!___derive.md](_!___derive.md). The open queue is [_!___workflow.md](_!___workflow.md). Every
word is [_!___terms.md](_!___terms.md); what the engine measures is
...
- **The absolute read on the behind row passes rule 1's trigger on peak leftover +7.46 %.**
  ...
  (4.55 % against 1.41 %); it is not the veto. The race call rests on 1.16's read of its picks:
  on this table its acted tickets read 49 ms and **12.0 %** ahead, so `seat.veto` alone passes it,
  flagged thin. Both known-dead anchors 5.2 passes are thin (sssssw +1.01 %, AbQcLH +1.69 %);
  the paying one is +7.46 %. That is the whole support of `seat.THIN_PEAK` today: one paying
  anchor, so a flag, not a line.
...
- **Applied to the mid-tape node, after the calibration** (its own members out of its public
  ...
  behind cost 4.55..14.45 %, peak leftover **+1.65 to +4.01 %** (buy >= 1 +4.01 %), so leftover
  exists (mid-tape-rule-1.md step 5.2d). 8dtx2t's burst start costs 5.39 %
  ...
  terms mvk >= 1.29 %, nstruct >= 3, sell_run <= 0. Occupancy of the family on every coin is red
  (re-read **-4.12 %** 0/6 at 83 ms, study cut, no floor). **7.1**: framed before its first buy
  **+2.24 %** 6/6; after **-3.36 %**; other coins **-3.89 %**. Coin property; next 7.3. The mint
...
name a public sell >= 1, not burst START, and that class keeps leftover behind their buy (1.27).
...
| AbQcLH burst start (hot-tape) | D none . E public burst start, a ~1 SOL buy after a dip . X cap15 and the bracket . seat lag_115 | lag p50 47 ms, we land ahead 5 %, -1.36 %/trade 1/7 on its picks (1.16); behind its buy cost 4.55 %, peak +1.69 % (1.27) | **a race on 1.16's read of its picks** (47 ms, 5 % ahead); on 1.27's leftover table 49 ms / 12 % ahead, so 5.2 alone passes it thin. Its picks book -1.36 % 1/7 at our seat |
| 49uohd sell >= 1 (hot-tape) | D none . E public sell >= 1 SOL . seat lag_115 | behind its buy cost 2.69 %, peak leftover +6.42 % (1.27) | **5.2 PASS on that class** (cost is not a kill). Its dip-buy picks stay open (1.16, workflow 1) |
