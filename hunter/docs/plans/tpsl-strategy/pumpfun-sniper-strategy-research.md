...
- **Bundle retained supply:** of that launch-slot wallet group, how much is **still held** at entry time (net buys−sells per wallet — reuse `net_token_amount_by_wallet_and_mint`). Large unsold overhang = **skip**. (Note: this bundle-retention idea is distinct from — and was never the same as — the simpler `p_entry_max_cohort_held`/`p_exit_cohort_ratio` launch-cohort feature that shipped in `tpsl_sniper_2` and was later removed; see `cohort-removal-plan.md`.)
