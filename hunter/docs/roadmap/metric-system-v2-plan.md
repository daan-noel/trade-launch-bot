...
- A stage move is persisted with its start (`StageMoved`, `record_stage_move`), and a
  restart resumes the same stage on the same clock
  ([restart-state-restoration](../plans/strategies/restart-state-restoration.md)).
