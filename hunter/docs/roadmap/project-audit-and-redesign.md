...
| **M2** | `mark_buy_submitted` DB persist held inside nonce slot | ✅ (2026-07-14; re-applied 2026-07-20 on engine path) | Engine `exec_real` `on_signed`: sync `SubmittedBuyJournal` first, then fire-and-forget `mark_buy_submitted` under a 250 ms timeout (never awaits PG before `send_transaction`). Regressed during the fingerprint-engine migration (unbounded await); restored. |
