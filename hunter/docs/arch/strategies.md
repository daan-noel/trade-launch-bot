...
**Matched + simulate = tiered fingerprint caches** (`lab/src/state/analysis_cache.rs` + `trading_core::strategies::match_keys`). The match predicate is a Rust closure (not SQL), so the first matched POST / simulate backtest runs `collect_matching_tokens` once per `(strategy_id, fingerprint_key, from, to)` — **not** per `rule_id`. Rules that share the same token-creation fingerprint (Stage‑1 filters only; exit/sizing/entry gates excluded) reuse one whole-table scan. The same key caches the lake history load (`candidate_cache.rs`); only the per-token entry/exit walk reruns when exit/sizing/entry params change. Finished sim rows are keyed by `(rule_id, sim_key, range)` in `sim_results` — unchanged config + window → instant replay (SSE only, no walk). Matched pages the cached mint set via `StrategyRepo::find_tokens_by_mints_paged` + shared `ENRICH_SELECT`. TTL/GC: 10 min. Frontend `fingerprintKey()` (`ruleColorGroups.ts`) mirrors the Rust canonical join (guard test in `match_keys.rs`).

**Positions table = Current run + Old runs** (`?scope=current|history` on `POST …/rules/{id}/positions` and `POST …/rules/{id}/positions/summary`). A rule's positions are split by run: `current` pages the rule's latest run (`StrategyRepo::latest_run` → `find_positions_by_run_paged` + `positions_summary_by_run`); `history` pages every prior run (`find_positions_by_rule_excluding_run_paged` + `positions_summary_by_rule_excluding_run`, excluding the latest run id), each row stamped with its `run_seq` from `run_seqs_for_rule` for a run column + per-run banding. Scope absent = legacy (paper = latest run, real = all runs). Both summary endpoints take the same `TableRequest` search/filters as the table (pagination/sort ignored) so the summary card tracks the filtered cohort. Both live (`live/src/api/handlers/strategies/positions.rs`) and lab (paper-only) handlers accept the param, and both attach the shared token enrichment to the page via `enrich_position_responses`/`enrich_positions` (one `fetch_by_mints` over the page's mints) — `PositionResponse` (the live-local + `trading_core` copies both) gained `symbol`/`ath_price` + `#[serde(flatten)] token: TokenEnrichment`, defaulted empty on the SSE-delta / single-position paths. Frontend: one shared `RunPositionsPanel` renders both sections — Current run keeps the live SSE/poll `useRulePositions` path; Old runs is a second `useRulePositions(…, live=false)` (fetch-only, immutable) and only shows when a prior run exists. Wired into all live + lab strategy pages, now consuming already-enriched rows (no `mergeTokenData`).

**Failed-exit PnL booking (row↔card SSOT).** A closed position is `End` (clean exit fill) **or** `ExitFailed` (the sell never landed — `mark_exit_failed` records a *hypothetical* exit price but **no** `exit_sol`/`exit_lamports`). Both are booked as a **full loss of deployed capital** (`pnl = −entry`) everywhere realized stats are summed: the SQL summary (`positions_summary_by_run` / `rule_counters_for_latest_paper_runs`, `SUM(COALESCE(exit_lamports,0) − entry_lamports)`) and the live in-RAM `RuleClosedStats::apply` (which mirrors it — `exit_sol.unwrap_or(0.0) − entry`). This keeps the **live rule-row** (fed by the runtime cache) and the **current-run summary card** (fed by the SQL) in agreement; the old `realized_pnl_sol()`-gated cache silently dropped `ExitFailed` from the PnL numerator+denominator (kept it only in the loss count), so the row showed a rosier return than the card. `best_pct`/`worst_pct` likewise book `ExitFailed` as −100% so the distribution tails match the PnL sums instead of the never-realized hypothetical price.

**Live rule-row counter source (active → cache, inactive → DB).** `rule_to_json` (`live/src/api/handlers/strategies/rules.rs`) sources the list-row counters by lifecycle: an **active** rule shows its **current run** live from the runtime cache (`closed_stats_by_rule` + the holding/pending/total counters — ticks as positions arm/fill/close); an **inactive** rule shows its **last run** from the DB via `StrategyRepo::rule_counters_for_latest_runs` (batched `DISTINCT ON (rule_id)` over each rule's latest run *in its own `trade_mode`*). This is necessary because the in-RAM `closed_stats` only warms active + `Running` rules on boot (`load_from_db` skips inactive rules and non-`Running` runs), so a stopped rule's row would otherwise blank out after a restart. The two agree when active (latest run == current run). The batched DB query shares its aggregate predicates with the lab paper counters via the `RULE_COUNTERS_SELECT` const (mirrors `positions_summary`), so row and card can't drift. `list_rules`/`get_rule` (the read paths) supply `db_counters`; the mutation endpoints pass `None` (the frontend refetches the list). The cache stays authoritative for entry-gating caps regardless.
