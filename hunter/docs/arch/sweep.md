...
| Driver (small groups) | `sweep_group_serial`: **token-outer** (series built once/token, combos folded in batches over it) when the full `n_combos × ComboAgg` set fits across all workers; else **batch-outer** fallback (bounded `batch × ComboAgg`, series rebuilt once/batch). Single-batch groups are token-outer either way. ⚠️ **Measured 2026-07-19: on a realistic multi-batch run the token-outer branch never fires** — 0 of 405 groups took it, the fallback took all 314 multi-batch groups at `n_batches=3`. The sweep's own RSS drives `usable_host_bytes()` to 0 mid-run, so the fit test always fails and the dominant series-build cost *is* multiplied by `n_batches` in practice. Evidence + fix options: [../plans/sweep/perf-measurements.md](../plans/sweep/perf-measurements.md) |
