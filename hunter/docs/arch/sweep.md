...
| `generic/` | `GenericSweepStrategy` — the one sweep family (Phase 7 retired the per-strategy tpsl/swing adapters). `axes.rs` = fingerprint + TP/SL + metric-condition axes → `RuleParams` combos; `strategy.rs` = `Strategy` impl (`TokenState = MetricSeries` precompute; `resolve_entry`/`resolve_exit` scan the series reusing the engine `evaluator` + shared `kernel` cost); `guard.rs` asserts the scan ≡ a single-token `run_replay` (the real fold) |
...
| Exit-scan path | **scalar (default)** or optional **AVX-512** (`generic::strategy::resolve_exit_simd`, 8×`f64`/instr `__m512d`) for the pure TP/SL first-exit search. Per-run toggle — the sweep form's *AVX-512* On/Off sends `use_avx512`, honored only when the host has `avx512f` (else forced scalar + a `sweep_notice` toast). **Byte-identical** to scalar (parity guard `guard::simd_exit_scan_matches_scalar_across_paths` + `strategy::tests::simd_*` kernel tests); vectorizes only the *search for the exit row* — the `round_trip_with_costs`/`RunAgg` money math stays the one shared `kernel` copy, and metric-condition exits stay scalar. Process-global (`registry::set_use_simd`, single-flight) + run-local (not persisted; a property of *how the box computed*). Lab-only. **Measured 2026-07-19** (release, 400 combos × 3 884 tokens, 1 lake day): scalar `sweep_pass` **0.63 s** → AVX-512 **0.29 s** = **2.2× faster** (repeatable, 3 samples/side). Cold end-to-end is Amdahl-capped by `corpus_load` (DuckDB, ~0.78 s) to ~1.3×; a warm-corpus tune-the-rule loop (cache hit, `corpus_load`→0) gets the full ~2.2×. **Release-only:** in a *debug* build the intrinsics aren't inlined and AVX-512 runs ~2.3× **slower** than scalar (measured), so the toggle only helps under `cargo build --release` / `cargo run --release` — a plain `cargo run -p hunter-lab` is debug and should leave it off. |
