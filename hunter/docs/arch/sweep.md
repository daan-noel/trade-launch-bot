...
| **`pnl_percent` is not notional-invariant.** The fixed per-transaction cost (`fixed_buy_sol` / `fixed_sell_sol`) does not scale with trade size, so PnL% is only comparable across runs at the *same* `buy_amount_sol`. | The notional *chain* itself is consistent; this is the residual of a fixed per-leg cost, not a bug |
