...
- **`ix_labels_filter` + `field_filters`** — the manual path: exact **ordered-sequence** label match (same on-chain order, same repeated labels, same length — the axis semantics `hunter_engine::fingerprint::matches` grades on, so `normalize_label_vec` never sorts or de-duplicates) and exact-value field pins. These cannot express a bucket axis, which is why scoping is a separate request field rather than a filter prefill. Ignored (and stored as `NULL`) whenever `fingerprint_id` is set.
  ...
  *Mask width.* Every `to_char` group-key mask carries `SQL_MASK_INT_DIGITS = 18` integer positions. This is not cosmetic: `to_char` renders `########` on overflow, and that string is a **valid TEXT group key**, so an overflow produces a *wrong group* rather than an error. 18 digits is what it takes to hold pump.fun's `max_cost_lamports = u64::MAX` "no slippage limit" sentinel (≈1.84e10 SOL) — never narrow it. **Still open:** `extract_lamports` reads that sentinel as `u64::MAX as i64 = -1`, so the Rust corpus side keys those tokens at −1 lamport while SQL keys them at their real magnitude — the two disagree for that value, and what the sentinel *should* mean (most likely `∅`/unset, since it encodes "no limit" rather than an amount) is an open product decision. Background: [`@history/2026-08-04-group-key-unit-drift.md`](../history/2026-08-04-group-key-unit-drift.md).
