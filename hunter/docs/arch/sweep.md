...
| Pricing (fill + cost model) | **Part of a run's identity, chosen per run.** `Pricing { buy_amount_sol, fill_model, cost }` threads from the request through `run_grouped` → `GenericSweepStrategy` → every scan fn. `fill_model` picks which trade in the window prices each leg (the same `FillModel` `ReplayConfig` threads — fill *eligibility* is identical across models, so the taken set never moves, only the price); `cost_model` picks `pumpfun_impact` (the default) vs `pumpfun_fee_only`. Both persist on the run row (migration `0010`) and the drill-in re-simulates under the run's own pair, for the same reason it re-uses the run's `as_of` (parity plan B7). `NULL` ⇒ the legacy pair (`worst_case` + the retired `pumpfun_default`), which is why the column is decoded with `CostModelKind::from_stored` and **never** with `Default` — a blank stored column and a blank request field are different questions with different right answers. **Fixed per-leg tip/priority** inside either cost model comes from process-wide `FeeTuning` (`JITO_MIN_TIP_SOL` + `CU_PRICE_MICRO_LAMPORTS` — same knobs live applies to the trader; lab installs at boot). **Why the flat-slippage model is retired:** a fill model already prices execution slippage, so charging `slippage_bps` again counts it twice — and since `fixed_cost_sol_per_leg` is per-leg, that haircut scales with how often a combo fires, i.e. it is *not* rank-preserving across combos. Nothing can select it; the FE still warns on a stored run that carries it, and a re-run of such a run comes back under `pumpfun_impact` (`selectableCostModel`) rather than reproducing an untrustworthy ranking. Guard: `assert_parity` runs scan ≡ `run_replay` under **every** `FillModel`. |
