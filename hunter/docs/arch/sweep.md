...
- **`ix_labels_filter` + `field_filters`** — the manual path: exact-set label match and exact-value field pins. These cannot express a bucket axis, which is why scoping is a separate request field rather than a filter prefill. Ignored (and stored as `NULL`) whenever `fingerprint_id` is set.
  ...
  **`field_filters` values on a bucketed axis are parsed by the one shared `hunter_engine::grouping::SolFilter`** — human SOL in two forms: an exact amount (`"1.515"`) or a half-open bucket range (`"1.5–1.6"`, the text a group chip renders, so a chip pastes straight into the box). Both are independent of the run's grouping width; a range is `[lo, hi)` so pasting a chip selects exactly `same_bucket` (locked by `a_bucket_label_round_trips_as_a_range_filter`). SOL is the unit every producer already emits (the "Max SOL cost" field label, the picker tooltip, Flow discovery's `solText` prefill). Consumers that must agree, each locked by a test: the resident-corpus retain `grouped_sweep::matches_field_filter` (shared by sweep / replay-inspect / metric discovery / flow discovery, `field_filter_tests`), the dashboard SQL `creation_stats_repo::field_filter_pred`, and the frontend's single `buildFieldFilters`. Which axes are bucketed is decided **once** by `GroupField::is_bucketed`, locked against `render_field` by `is_bucketed_agrees_with_render_field`.

  **Grouping precision is `SolPrecision`**, separate from the filter: `Bucket(width)` (default) or `Exact`, the latter keying each SOL axis on the amount itself for "what are the most common exact dev-buy sizes?", which bucketing by construction cannot answer. Exact is a **named mode with its own flag** (`exact_sol` on the wire, an "Exact values" checkbox in the picker) and never a width of `0` — a width is a measured quantity, `floor(v/0)` is a division by zero, and a magic-`0` width already caused one live `bucket_size_amount` bug. `SolPrecision::width()` returns `None` in exact mode, and the response echoes `bucket_width: null`, which is what the client branches on. **An exact card cannot become a fingerprint** — the live engine matches SOL axes with `same_bucket` at the fingerprint's own `bucket_size_amount`, so any width invented for the card would arm on a window the card never showed; Create is hidden with that reason on the card instead. `exact_sol_label` (Rust) and `sol_exact_sql` (SQL) are byte-for-byte mirrors, as `bucket_sol_label`/`sol_bucket_sql` are.

  *Mask width (2026-08-04).* Every `to_char` group-key mask now carries `SQL_MASK_INT_DIGITS = 18` integer positions. `to_char` renders `########` on overflow, and that string is a valid TEXT group key — so an overflow became a *wrong group*, not an error. The old 8-position mask did that to every token carrying pump.fun's `max_cost_lamports = u64::MAX` "no slippage limit" sentinel (≈1.84e10 SOL): 11,250 in a 30-day window, all collapsed into one `########.#` group that silently disagreed with `bucket_sol_label`. **Still open:** `extract_lamports` reads that sentinel as `u64::MAX as i64 = -1`, so the Rust corpus side keys those tokens at −1 lamport while SQL now keys them at their real magnitude — the two still disagree for that value, and what the sentinel *should* mean (most likely `∅`/unset, since it encodes "no limit" rather than an amount) is an open product decision.

  *History (2026-08-04).* The three disagreed and the filter was unsatisfiable on both backends: the sweep compared the raw **lamports integer** while the dashboard compared the rendered **bucket label** (`'1.5–1.6' = '1.515'`), and the frontend sent human SOL to both — with `parseNumbers` rejecting the range syntax that could have matched the dashboard. This is the same shape as the SSOT traps in the product `CLAUDE.md`: one wire field, several readers, each silently reading it in its own unit.
...
Before this (≤2026-08-04) each consumer re-derived it from `group_key` alone, and every derivation lost clauses in the **widening** direction: `field_filters` were dropped with a `warn!`, a scoped run that also grouped by extra fields promoted the *scope* (wider than the group), a `∅` key dropped the axis (so the rule matched tokens that HAVE a value), and `token_program_id` / `is_cashback_enabled` had no axis to land in. Each shipped a rule that armed on a superset of the tokens the promoted numbers came from.

**Promote fails closed.** A fingerprint axis is a scalar plus one *fingerprint-wide* bucket width, so some selections have no expression: an absent (`∅`) axis, a multi-value or arbitrary-range pin, the `u64::MAX` "no cap" ceiling, the two axes with no column, and any run needing two different widths at once (an exact `field_filters` pin alongside a bucketed group-by). `materialize` returns those clauses and the endpoint answers **400 naming them** — it never drops a clause to produce a wider gate. The groups response carries the same verdict (`promotable` + `blockers`) so the card can say so before the click. Widening this coverage means giving the axis its own predicate (`eq | range | in | absent`) in `hunter_engine::fingerprint` + the `fingerprint_scope_clauses` SQL mirror — one evaluator and one emitter, per the sentinel rule in the product `CLAUDE.md` — not loosening this seam.

The scope is persisted on the run row (`grouped_sweep_runs.fingerprint_id`, `lab/migrations/0001_init.sql`, no FK) because it is not reconstructible from the filter columns: the token-results reload re-applies the same match and re-run restores it in the form. Promote reuses the **saved row itself** when the group is the whole scope unnarrowed (`is_scope_only` — materializing would re-anchor a bucketed axis on its bucket's lower edge, match-identical but a different `find_or_create` identity, i.e. a duplicate); a group that narrowed the scope promotes to its own narrower fingerprint, carrying the scope's `metric_config`.
