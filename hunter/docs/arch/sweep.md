...
| Exit-scan path | **Prefix-extrema index (default)** — `generic::exit_index::ExitIndex` builds per-`(token, entry)` monotone `hull_max`/`hull_min` once (when the entry cache goes stale) and resolves each pure-TP/SL combo in O(log n) via `partition_point`. Works in debug and release, any CPU; scratch is transient (recycled Vecs, not cached across tokens) so peak RSS is unchanged. Tie-break **Dead > StopLoss > TakeProfit**; `has_exit_metrics` / `NoEntry` fall back to the linear scalar `resolve_exit` (parity reference, never deleted). Optional **AVX-512** toggle still selectable (`resolve_exit_simd`, 8×`f64`) for A/B — form `use_avx512`, honored only with `avx512f`. **Byte-identical** to scalar (guards `index_exit_scan_matches_scalar_*` + `simd_exit_scan_matches_scalar_across_paths`). Money math stays the one `kernel` copy. Lab-only. AVX-512 measured 2026-07-19: scalar linear **0.63 s** → SIMD **0.29 s** (2.2×, release); SIMD is ~2.3× *slower* in debug — leave the toggle off under plain `cargo run`. |
