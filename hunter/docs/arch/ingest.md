...
**Push hooks (`ingest_core::PushHooks`, optional):** the same subscription can carry a `blocks_meta` filter and an `accounts` filter for host-chosen pubkeys; the two callbacks run on the supervisor task (cheap parse + store only). Hunter's `main.rs` bridges block metas → `Engine::set_cached_blockhash` (blockhash cache, 0 steady-state `getLatestBlockhash`) + `ingest::feed_lag::FeedLagGauge` (below), and nonce-account updates → `Engine::on_nonce_account_update` (durable-nonce push re-arm) + the wallet balance cache. Hosts that don't opt in (forge) get a byte-identical subscription. Push updates do not feed the idle watchdog on a scope that carries the program — there it guards the *transaction* stream, which a firehose never leaves quiet. On a pools-only scope they are the only liveness signal there is, so `idle_basis` judges by any frame instead; see `@plans/ingest/reconnect-restart-flow.md`.

**A block meta rides a transaction subscription; it never justifies one.** `build_subscription` asks for `blocks_meta` only when the subscription carries transactions — for a server-filtered wire, only when `account_include` is non-empty. One block meta per slot is ~2.5 metered frames/s forever, and the cache it fills is read by the AMM buy path alone (`build_recent_tx`; curve sends ride a durable nonce). A gRPC feed with no account to watch has no AMM pool tracked, so the frames fill a cache nothing reads: under `CURVE_SOURCE=nats` with no tracked pool they are the entire provider bill. The `accounts` filter is not gated the same way and is what holds that subscription open (`PushHooks::wants_stream`) — those pubkeys are the host's own, they update only when the bot itself transacts, and they keep the durable-nonce slots re-armed at feed speed rather than at post-send poll speed.

**The tracked pool set has exactly one owner: `live/src/ingest/pool_reconcile.rs`.** Three call sites add to the index for unrelated reasons — the migration decode auto-registers, `HeldPoolGate` subscribes a held position's pool, `token_sync` registers a synced migrated token — and every one of them is a *hint*. The reconciler decides, every 30 s, from scratch:

```text
keep a pool when
    the mint has an unsettled position in ANY mode    (strategy_repo::distinct_unsettled_mints)
 OR track_post_migration is on AND the pool is live   (POOL_SUBSCRIBE_ACTIVITY_WINDOW_SECONDS)
 OR it entered the index less than GRACE (5 min) ago
```

Both directions are applied. It **subscribes** a bag whose pool nothing tracked (a missed migration event, a position adopted across a restart) — an exit-path fix on its own — and drops pools nothing asks for, which is what stops the metered filter growing for the life of the process. The retention arm is re-read from `strategy_positions` on every pass rather than from the accumulated `HeldPoolGate` events, which is what makes pruning safe: the set it checks against is the same rows the exit path is driven by, so a drifted in-memory set is repaired in the direction of *more* subscription. `HeldPoolGate::replace` is that repair. A pass that cannot read the DB changes nothing.

Retention spans **paper as well as real**. `HeldPoolGate` stays real-only — it answers "could the wallet be holding a bag", the right question for RPC-warm and sell-confirm — but a paper exit resolves from the same `trades` rows a real one does, so a paper position's pool is retained too.

Losing the block metas hands the blockhash cache back to its watchdog, so `CacheCfg::blockhash_refresh_ms` (15 s) and `blockhash_max_age_ms` (20 s) are what keep that from re-appearing as `getLatestBlockhash` traffic. The watchdog skips a tick only while the cache is fresher than **half** the interval: testing the full interval makes its own last write borderline-fresh, so the cache drifts to two intervals, past the read tolerance, and puts a live fetch back on the AMM buy hot path. `config::tests::refresh_interval_stays_inside_the_read_tolerance` pins the pair.
