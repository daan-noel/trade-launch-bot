...
| `handlers/tokens/metric_series.rs` | `GET /api/tokens/{mint}/metric-series` — every metric's value at every **event** (rule-authoring chart panes). Folds through the shared sparse tick grid (`hunter_engine::metrics::grid`), same as the sweep precompute: a trade-only fold mis-samples every time-decaying metric. Bounded by `MAX_SERIES_ROWS`, reporting `truncated`/`covered_until`. Seeds the static token facts (`ix_count`, `prior_launches`, the creator wallet) off ONE `tokens` read before the first fold, and loads the lake's wallet column whenever a recorded metric is wallet-keyed (`MetricId::needs_wallet_identity`) — an unseeded fact reads `NaN` for the whole token and an identity-less one reads every trade as one wallet, both of which draw a blank pane rather than an error |
