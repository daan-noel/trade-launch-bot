...
| `jito_tip.rs` | `JitoTipCache` — bg-refreshed tip-floor; max(percentile ladder, `min×1.5^level`), clamped `[MIN,MAX]` (default MIN=`0.001` Sender Max) |
...
- **Strategy integration:** the engine adapters call `buy_token_snipe_write_ahead` / `sell_token_once`/`amm_sell` with `confirm=false`; fill confirmation comes from the `trades` LaserStream feed, never a new RPC call. Pass-1 `BuySubmitted`/`ExitPending` PG is async so submit spawn is not gated on durable writes.
- **Zero-RPC AMM pool warmup:** `observe_amm_swap_accounts` harvests a migrated token's pool facts (`AmmPoolInfo` incl. the fee-share marker, optional `pool_v2`, and the creator-vault pair) from one observed on-chain swap's account list — pure CPU, fed inline by hunter's ingest consumer. Requires the mint's pool to stay on the LaserStream filter (held-position retention via `HeldPoolGate`, independent of `track_post_migration`). The old RPC `prewarm_amm_pool` is deleted; `fetch_fee_share_marker` remains only as the cold fallback inside `amm_pool_info` (limit 5, sequential, early-exit, singleflight per mint). `refresh_amm_pool_info` (2006 heal) re-reads the pool account only and reuses a cached fee-share marker — it must not re-burst `getTransaction`. AMM ATA resolve uses mint-filtered `getTokenAccountsByOwner` (same as curve), never a full wallet scan. `amm_config` (fee bps) is stale-while-revalidate — trades never block on its refresh once primed.
- **AMM `pool_v2` (Apr-2026):** when the pool's `coin_creator` is set, swaps append PDA(`["pool-v2", base_mint]`) readonly immediately before the buyback-fee pair. Omitting it reverts with `InvalidPoolV2` (6062 — live name; published IDL still says `BuybackFeeRecipientMissing`). Cashback swaps that need `pool_v2` do **not** also carry the legacy `PUMP_AMM_CASHBACK_GLOBAL` marker. Harvest accepts any buyback-fee whitelist member at the trailing slot (live swaps rotate).
