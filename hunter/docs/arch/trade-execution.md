...
- **Zero-RPC AMM pool warmup:** `observe_amm_swap_accounts` harvests a migrated token's pool facts (`AmmPoolInfo` incl. the fee-share marker, optional `pool_v2`, and the creator-vault pair) from one observed on-chain swap's account list — pure CPU, fed inline by hunter's ingest consumer. The old RPC `prewarm_amm_pool` (`getSignaturesForAddress` + `getTransaction` bursts) is deleted; `fetch_fee_share_marker` remains only as the cold fallback inside `amm_pool_info` (limit 5, sequential, early-exit). A round-trip guard test pins the harvest parser to `amm_swap_accounts`' builder layout; any layout drift fails safe to the cold path. `amm_config` (fee bps) is stale-while-revalidate — trades never block on its refresh once primed.
