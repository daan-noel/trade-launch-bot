...
- `build_breadth_day_stats` (0017) — one row per `(day, recipe)`: distinct wallets that bought with a build recipe (ordered `ix_labels` without setup, teardown and memos), on any token, on the PREVIOUS UTC day. Computed once a day off one day of curve buys in `trades`, written once and never rewritten; the engine stamps each buy with its recipe's count, and `m_holder_book.public_app_share` classes a first buy above 100 as a public app. The recipe is stored as labels and hashed at load by `flow_ix::build_hash`, never in SQL.
...
| `build_breadth_repo.rs` | build_breadth_day_stats (+trades) | `compute_day` (one `GROUP BY ix_labels` over a day of curve buys returning each sequence's distinct `wallet_id`s; cut to recipes and unioned per recipe in Rust; an error on a day `trades` does not hold), `insert_day` (`ON CONFLICT DO NOTHING`), `load_day`, `load_or_compute_day` (the one call live's daily refresh and simulate make), `to_engine` (recipe -> `BuildBreadth` through the engine hasher) |
