...
- **Lab-only** - `lab/migrations/` (`0001_init.sql` = the `grouped_sweep_*` tables + the covering `idx_trades_wallet_time`, likewise squashed; `0002` = `ix_pattern_sets`, the Trader Analysis flow lens; `0005` = lens `kind` + `working_templates` + fee pins on exact rows; `0006` = metric system v2: `grouped_sweep_runs.ix_patterns` / `scale_out` / `scale_out_top_k` become `tags` / `stage_plans` / `stage_plans_top_k`). Runner: `lab::storage::lab_migrations::run()`, called from `lab/main.rs` after `connect()`. Tracked in a lab-private **`_lab_migrations`** ledger (its own checksum table; reuses `migrate!` only as an embedder, never `.run()`, so core's `_sqlx_migrations` is untouched). Run by **`lab` only** -> these tables never exist on EC2. Add a lab-only table = drop `NNNN_*.sql` into `lab/migrations/`.
- **Data migrations** - row rewrites SQL cannot express (a stored JSON document converted by the engine's own code), run once per database right after the schema chain, each in ONE transaction, and recorded by name in the core table **`_data_migrations`** (`0021`; the lab's rewrites share it, since the lab tables live in the same database). Core: `storage::data_migrations::run` (called from `connect()`) - `metric_system_v2` converts fingerprint `tags` + `criteria`, rule `params` and run `params_snapshot` through `hunter_engine::v1`, validates every converted tags document, and clears each `Running` run's `config_hash` (it digests the v1 JSON); a fingerprint or rule that fails aborts the whole transaction naming the row, a run snapshot that fails stays as written. Lab: `lab::storage::lab_data_migrations::run` - `lab_metric_system_v2` converts each `grouped_sweep_runs` row's `axes_spec`, `tags` and `stage_plans`; combo `params` and group `best_params` stay as written, because every reader goes through `hunter_engine::v1::parse_params_any`. `hunter-lab migrate-v2 --dry-run [--database-url <url>]` runs the core conversion in a rolled-back transaction and reports what it would change; it reads `tags` or `metric_config`, so it checks a database the schema chain has not reached.
...
- `trades` *(TimescaleDB hypertable on block_time, ~1mo retention)* — mint, wallet, trade_type, amount_lamports(BIGINT) / token_amount(BIGINT raw units), reserve_lamports/reserve_token(BIGINT venue-neutral priced pair: the curve's virtual reserves; on amm rows the quote vault + PumpSwap's virtual quote `PUMP_SWAP_VIRTUAL_QUOTE_SOL` and the base vault - an amm row written before the decoder priced with it holds the bare vault), tx_signature(BYTEA), slot, block_time, venue(`curve`/`amm`), **`ix_labels`(JSONB, migration 0002, forward-only)**, **`fee_lamports`(BIGINT NULL, migration 0005, forward-only)**, **`cu_limit`/`cu_price`/`tip_lamports`(BIGINT NULL, migration 0013, forward-only)**, **`payer_id`(INTEGER NULL) / `is_proxied`(BOOLEAN NULL, migration 0014, forward-only)**, **`payer_net_lamports`(BIGINT NULL, migration 0019, forward-only)**, **`venue_fee_bps`(REAL NULL, migration 0020, forward-only: the fee a PumpSwap swap charged over the pool's constant-product amount; NULL on curve rows)**; price derived in `trades_priced` view (`price_per_token` = SOL/token). PK `(block_time, tx_signature, leg_index)`. **This table = the LaserStream feed.**
...
- `build_breadth_day_stats` (0017, 0018) — one row per `(day, recipe)` for every build recipe (ordered `ix_labels` without setup, teardown and memos) bought with on the PREVIOUS UTC day: its app's distinct buying wallets (`app_buyers`) and buy transactions (`app_buys`), on any token, across every recipe of that app. The app is `flow_ix::recipe_app` (the first program past compute budget, system, token, associated-token and memo), the recipe itself for a direct pump.fun call. Computed once a day off one day of curve buys in `trades`, written once and never rewritten; the engine classes each row with `holder_book::is_public_app` (> 100 wallets and >= 2 buys per wallet) and stamps each buy with its recipe's class for `m_holder_book.public_app_share`. The recipe is stored as labels and hashed at load by `flow_ix::build_hash`, never in SQL.

- `fingerprints` - one `criteria` JSONB map of axis -> inclusive integer range / label sequence (migration 0009; the per-axis columns and the row-wide bucket width are gone); **`tags` JSONB NOT NULL DEFAULT '{}'** (core `0021` renames it from `metric_config`) - the fingerprint's named trade lists, `{ "<tag>": { "match": {...}, "side"?, "sticky"?, "exclude_creation_slot"? } }`, validated by `hunter_engine::metrics::tags::config::validate_tags` and compiled per reload by `compile_tags`. Part of ROW identity for `find_or_create` and the `fingerprints_identity_uniq` index, though NOT of match identity: it selects no token, but it defines what every `@tag` read of that row's rules counts, so two rows matching the same tokens with different tags are different fingerprints (the eleven `8dtx · <router>` carriers share `{}` + `wildcard` and differ only here).
...
| `trade_repo.rs` | trades | `find_fill_by_signature`, `sum_legs_by_signatures` (per-sig attribution), `avg_entry_by_wallet_and_mints` (**manual-buy cost-basis SSOT** → `AvgEntry {avg_entry_price (SOL/raw), total_token_amount, total_cost_lamports, wallet_paid_lamports}` per mint, `SUM(amount_lamports)/SUM(token_amount)` over `trade_type='buy'`, plus −Σ `payer_net_lamports` once per transaction (NULL if any buy lacks it) - the basis the Holdings view books, pro-rata to the tokens held; bounded by the held-mint set; feeds the portfolio service), `wallet_traded_mints` (**Trader Analysis** → `WalletTradedMint {wallet_address, mint_address, first_trade_at, last_trade_at, buy_count, sell_count, buy_sol, sell_sol, buy_token_amount, sell_token_amount, entry_at, entry_slot, entry_tx_index, exit_at, exit_slot, exit_tx_index, entry_curve_sol, exit_curve_sol}` per distinct mint the wallet traded in a `since..=until` `block_time` window (`until: Option` — `None` ⇒ open-ended, bound as a NULL `$3::timestamptz` so one SQL string serves both shapes; EVERY aggregate below is scoped to the same window, so a closed upper bound reads the wallet exactly as it looked at that instant), `GROUP BY wallet_id, mint_address ORDER BY MAX(block_time) DESC` + optional `LIMIT n` (`limit<=0` ⇒ unbounded, the page default) + `COUNT(*)`/`SUM(amount_lamports)`/`SUM(token_amount)` each `FILTER (WHERE trade_type=…)`; both buy+sell count; the entry (first **buy**) and exit (last **sell**) legs come from `(ARRAY_AGG(col ORDER BY slot, tx_index, leg_index) FILTER (WHERE trade_type=…))[1]` — per-side first/last without a second scan, ordered by the execution key, not `block_time`. Each leg's `reserve_lamports` and own `amount_lamports` go through `pre_trade_real_sol` → the depth **before** that leg landed (the reserve snapshot is post-trade, so a leg's own impact is inside it) and then the `approx_real_sol_reserves` SSOT, giving `entry_curve_sol`/`exit_curve_sol`; the handler divides by `PUMP_GRADUATION_REAL_SOL` for the wire `wallet_*_curve_pct`. All four are `Option` — an exit-only window has no entry, a still-held bag no exit, and a missing reserve snapshot reads unknown, never 0; empty for an unknown wallet; feeds lab `GET /api/wallets/:wallet/tokens` (`?days=N` for the rolling window, or `?from=&to=` RFC3339 for an explicit range — `from` present ⇒ `days` ignored, `to` alone anchors the rolling span to that instant; `resolve_window` swaps a reversed pair and clamps the span to 90d, keeping the UPPER bound), which merges each with the full token row via `TokenRepo::find_list_rows_for_mints` → `WalletTokenRow` = flattened `TokenSummary` + `wallet_*` curve-side activity + the wallet's `episodes` from `wallet_txs_on`. Rides the **lab-only** covering index `idx_trades_wallet_time (wallet_id, block_time DESC, mint_address, trade_type)` — index-only, zero EC2 ingest cost. The entry/exit legs also carry their `(slot, tx_index)` tape position: `block_time` is second-precision and ties across a whole slot, so it is the only key that can order two wallets' entries against each other), `wallets_traded_mints_on` (**Trader Analysis co-trade** → the SAME rollup for a SET of wallets restricted to an explicit `mints` slice, `GROUP BY wallet_id, mint_address`, unbounded — the caller already bounded the mints. Both readers share ONE private `traded_mints_agg` (and so one SQL string), which takes the resolved `wallet_dict` id→address map, an optional `$4::text[]` mint scope and the `LIMIT` bind; `limit` is meaningful for a SINGLE wallet only, since across several `ORDER BY last_trade_at DESC LIMIT n` would cut through the union. Untracked addresses and `(wallet, mint)` pairs with no leg in the window drop out — never a zero row. Feeds `?with=<csv>` on `GET /api/wallets/:wallet/tokens`, capped at `MAX_COMPARISON_WALLETS`=8; absent `with` ⇒ not one extra query. The handler measures each comparison entry against the primary's → `CoTrader {entry_lag_slots, entry_lag_tx, bucket}`, `bucket` = `co-slot`/`leads`/`follows`/`independent` by `CO_TRADE_FOLLOW_SLOTS`=3 — same-slot means both wallets reacted to the same tape event, NOT that one copied the other. A co-trade DB error degrades to the single-wallet page rather than failing the read), `wallet_txs_on` (**Trader Analysis episode ledger input** → `(mint, WalletTx {slot, tx_index, time_ms, token_delta, flow_lamports})` for one wallet on the page's mints, legs collapsed per `(mint, slot, tx_index)`, tape order per mint; mint-scoped, so it rides `idx_trades_mint_order`. `flow_lamports` = `payer_net_lamports` only when the wallet paid (`payer_id = wallet_id`) and no leg of another mint or wallet shares the transaction — a `(block_time, slot, tx_index)` lookup inside a `CASE`, so it runs only where a flow exists: ~0.15 ms a transaction on an open chunk, ~3 ms on a compressed one; otherwise `None`, never a curve-side substitute. Feeds `strategies::wallet_ledger::wallet_episodes`), `prints_in_slot_windows` (**Trader Analysis pre-entry probe** → `TapePrint {mint_address, slot, tx_index, wallet_address, is_buy, amount_lamports, ix_labels, cu_limit, cu_price, tip_lamports}` for every leg inside each caller-supplied `SlotWindow {mint_address, lo_slot, hi_slot, lo_time, hi_time}`: one `UNNEST($1::text[], $2::bigint[], $3::bigint[])` triple joined to `trades`, i.e. a nested loop of one `idx_trades_mint_order` range per window. The `block_time` bounds are the SPAN of the passed windows, never per mint — slot is the precise filter and a constant time range is what lets the planner exclude chunks before the loop starts. `exclude_wallet` drops one address in SQL (`$6::int IS NULL OR wallet_id <> $6`) so its legs never reach a count or a sum; an unknown address excludes nothing. LEGS, not transactions — one tx emits several, callers collapse on `(slot, tx_index)`, and a `leg_index = 0` filter here would drop later-leg buys. Empty window list ⇒ no query. Feeds lab `POST /api/wallets/:wallet/pre-entry-ix`, which chunks 300 mints a query, one at a time), `tape_floor` (the oldest instant `trades` can still answer for = `MIN(range_start)` over `timescaledb_information.chunks` for the hypertable — milliseconds, and exact for that question because retention drops WHOLE chunks, where `MIN(block_time)` costs seconds and grows with the table. A probe window reaching below it is `unknown`, never "no match"), `for_each_seed_mint` (cold-start seed: the `TRADE_HISTORY_COLUMNS` projection ranked per mint, each row converted like a history read, so a seeded cache row carries the labels, fee trio and rebuilt `real_reserve_sol` a live one does; `SeedAgg` is the in-window count and volume only, the newest trade's facts come off the run's last row), `sig_bytes_to_base58` (`pub`; BYTEA→base58, reused by the lake export), `find_by_mints_all` (batched per-mint grouped reads; **reconstructs** the dropped `real_reserve_sol` via `approx_real_sol_reserves(reserve_sol, venue)`, never the live path — the **simulate/backtest path does not call it**: single-rule simulate reads the Parquet lake like the sweep, which bakes that same value in at export/read. Retained for ad-hoc grouped reads / tests) |
...
| `build_breadth_repo.rs` | build_breadth_day_stats (+trades) | `compute_day` (one `GROUP BY ix_labels` over a day of curve buys returning each sequence's distinct `wallet_id`s and distinct `tx_signature` count; cut to recipes and pooled per app in Rust, wallets unioned and buys summed; an error on a day `trades` does not hold), `insert_day` (`ON CONFLICT DO NOTHING`), `load_day`, `load_or_compute_day` (the one call live's daily refresh and simulate make), `to_engine` (recipe -> `BuildBreadth` through the engine hasher) |
| `ix_pattern_set_repo.rs` (lab) | ix_pattern_sets | `list` / `find` / `insert` / `update` / `delete` + `sanitize_patterns` / `sanitize_templates` / `validate`. Analysis-owned pattern sets - the Trader Analysis flow lens' twin of a fingerprint's lists, for tokens that belong to no cohort. `kind` is insert-only (`exact` \| `templates`); the picker is the switch. Exact `patterns` are `[{ group, ix_labels, cu_limit?, cu_price?, tip_lamports? }]` - identity is labels **plus** pins (same catch-all-vs-pin rule as a tag's `ix_shape` matcher); the same sequence under two group labels collapses to one. Templates store grain ids in `working_templates`. `MAX_PATTERNS` caps either list at 500. Unique on `lower(name)` -> a name clash is a 409, not a 500. Lab-only: never on EC2 |
...
| `fingerprint_repo.rs` | fingerprints | `find_or_create` (`criteria` + `wildcard` + `tags` - **not** `name`), `insert`/`update`/`list`/`delete`; persists `tags`; `name` is `Fingerprint::auto_name` when blank or a retired generator shape, and `list`/`find` rewrite those leftover labels in place |
