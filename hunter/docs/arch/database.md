...
- `grouped_sweep_results` — per (group, combo): score, win_rate, PnL metrics, exit-reason mix (incl. `n_exit_metrics`). Retention-filtered to ~660 rows/group max. The retired swing1 `n_exit_next_kill` counter was dropped in `lab/0002_drop_n_exit_next_kill.sql`
