...
export const STRICT_PARAM_HELP: Record<string, HelpTip> = {
  ...
  slice_size_prints: {
    ...
    body: [
      ...
      'the constant 100 * burst / window on every tape. It carries no information;',
      'read the OTHER m_flow_window metrics (gross_flow, buy_share, unique_wallets)',
      'over a print window instead, where the count is the denominator and the SOL is',
      'what varies.',
      '',
      'Both axes of the group must use the SAME unit, and burst <= window.',
    ].join('\n'),
  },
  window_size_sec: {
