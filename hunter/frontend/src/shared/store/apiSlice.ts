 ...
 * live / lab — so the frontend split (two Vite entries) can bundle only
 ...
 * (`sharedApi` / `liveApi` / `labApi`).
 */
export { baseApi as apiSlice, apiErrorMessage } from './baseApi';
export * from './sharedEndpoints';
// Mode endpoints are NOT re-exported here — live hooks live in
// `@live/store/liveEndpoints` and lab hooks in
// `@lab/store/labEndpoints`, imported directly by mode-only code so
