...
// Charts-grid overlays: tpsl1 tables mark entry/exit only (no swing legs). Matched
...
export function Tpsl1Page() {
  ...
  const handleSave = async (unlocked: LockState) => {
    ...
    try {
      if (editRule) {
        ...
        if (fpChanged) {
          invalidateStrategyMatched(dispatch, { strategy: 'tpsl1', ruleId: updated.id });
  ...
  const handleSimulateAll = useCallback(async () => {
    ...
    try {
      const res = await startSimulateAll('tpsl1', analysisRange);
