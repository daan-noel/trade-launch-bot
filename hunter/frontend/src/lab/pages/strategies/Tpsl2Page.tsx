...
import { ruleColumns as allRuleColumns, RuleRowProvider } from 'components/tpsl2/ruleColumns';
...
export function Tpsl2Page() {
  ...
  const {
    ...
    paperResult?.ruleId ?? null, rules, fetchTpsl2RulePositions,
    fetchTpsl2RulePositionsSummary, 'tpsl2', paperPosQuery, POSITION_NUMERIC_COLS,
  );
  ...
  // (`useRulePositions` above, scoped to the paper rule) — it live-patches its
  ...
  const {
    ...
    matchedBody,
    (body, signal) =>
      fetchMatchedPage('tpsl2', matchedRuleId ?? '', body as TableRequestBody, signal),
  );
  ...
  const handleSimulate = useCallback(
    async (rule: RuleRecord) => {
      ...
      setSimLoading(true);
      try {
        await startSimulation('tpsl2', rule.id, analysisRange);
      } catch (e) {
  ...
  const handleSimulateAll = useCallback(async () => {
    ...
    try {
      const res = await startSimulateAll('tpsl2', analysisRange);
  ...
  const onSelectPaperToken = useCallback(
    (key: string | null) => {
      ...
      setInspect(row ? { table: 'paper', key: row.id, target: inspectFromPosition(row) } : null);
    },
    [paperPositions],
  );
