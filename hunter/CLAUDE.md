...
| Landmine | The rule |
| --- | --- |
| Backtest honesty | **Two selectable cost models, and `pumpfun_impact` is the default** — the only kind that charges our own price impact, so the only one whose cost responds to buy size. `pumpfun_fee_only` is a zero-impact upper bound, for the "is there any edge at all?" screen. The flat-slippage `pumpfun_default` is **retired**: it double-counts what the fill model already prices and its error changes sign with buy size, so it does not preserve ranking. It stays decodable for stored runs only — read a persisted `cost_model` through `CostModelKind::from_stored` (blank ⇒ legacy), never through `Default` (blank ⇒ `pumpfun_impact`); the two fallbacks answer different questions. Fee **125 bps/leg**; cost is U-shaped, so the optimal fixed buy is `sqrt(fixed_per_leg × vsol)`. Runs stored before 2026-07-28 are priced at 100 bps with no impact charge and do not compare. <!-- pt-ok: cutoff, those runs are still in the DB --> [costs](docs/plans/strategies/execution-costs.md) |
