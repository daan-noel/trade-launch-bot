...
A red occupancy book with green leftover is 6.1 while he takes a tiny share of the class
(E is not spelled; do not freeze E and jump to D). It is 7.1 (the door is missing) only
after 6.1 has named which prints. Not phase 8 and not a dead E. E is this print (re-entry
is R). Study fires stop where the holdout starts.
...
- **What a rule TRANSACTS AGAINST is the pool state, and that is what every metric folds.**
  ...
  the readout), so `m_price` (life and windowed) and `m_position` read one series.
...
- **Price impact is charged on the PRICED reserve (`vsol`), never the real one.**
  ...
  Priced that way, simulate reads 4.62 pp a trade instead of 0.66 pp.
...
| Exit conditions that lie | `m_price.stall_sec` is *seconds since the last all-time high* - it caps every hold and doubles as an entry filter. Use `m_position.held_sec` (or a `held_sec` stage deadline) for a time stop, and arm a trail with a stage entered by a pnl line (`m_position.pnl_pct >= X -> go armed`, the trail a line of `armed`) or a `pnl_pct >= X` condition on the trail's own line: an ungated `m_position.retrace_pct` is a hard stop from entry, because the peak seeds at the fill. [armed-trailing-stop](docs/plans/strategies/armed-trailing-stop.md) |
| Restart rebuilds state, never re-decides it | Cached trades older than the loop's `started_at` prime the fold and emit nothing; deciding on them prices at the old trade and fills at the live one. Inversely, an adopted position with no new prints holds `NaN` — which satisfies no exit at all — so priming retries until the seed lands. Only prices the position lived through may move `peak`/`trough`. A rule switched on mid-run gets every live token born since `started_at`, rebuilt from its whole history; a token that crossed a restart stays out (the downtime's trades are nowhere). [restart](docs/plans/strategies/restart-state-restoration.md) |
...
| Trailing windows are O(1) | Never rescan the buffer inside a `value()`, and never assume the caller evicted at `now` - and give every windowed buffer family (flow, price, crowd, ix shapes, each `TagState`'s span windows) an arm in **both** `TokenTrack::on_trade` and `TokenTrack::on_tick`, or its buffers only ever shrink on their own next match. Reads are corrected at both window ends, so a missing tick arm is invisible in the numbers and shows only as retained state. Trade identity hashes (ix shape, marker bits, wallet) come from the ONE `metrics::trade_keys` hasher set, applied at load offline. [metrics](docs/plans/strategies/_!___metrics.md) |
| A loaded fingerprint is a cost on EVERY token | Every (fingerprint, tag) a loaded rule reads opens one `TagState` per token (`TokenTrack::tags`, keyed by `(FingerprintId, TagKey)`) and folds it on every trade of that token, so the working set - not the `fingerprints` table - is what may reach `EngineState`. `reload` narrows `fps` to the ids the rules name and compiles each one's `tags` ONCE (`compile_tags` into `EngineState::fp_tags`), and `TrackRequirements` registers only the tags a rule reads; deriving patterns where they are *used* re-walks the JSON and re-hashes every label sequence on the create fast lane, which is the serialized decision loop. Measured at 115 rows / 59 configured: 461 us per `TokenCreated`, of which 415 us was paid with **zero active rules**. |
| A registered metric with no compute arm is always-false | `TokenTrack::value` routes by family and is exhaustive, but each compute module's own `value` ends in `_ => f64::NAN` (as does a tag or span with no registered state), and `NaN` satisfies nothing - so a metric in the registry (`METRICS`) whose arm was never written is a gate that silently never fires. `engine/tests/every_metric_is_live_reachable.rs` walks `METRICS` and reads every metric, on every tag and span it accepts (life; seconds, slots and prints windows; slices; since age), through `reduce` + `read_state`; a new metric, tag level or span kind is covered without touching it. |
| A DashMap guard at an `.await` wedges the process | dashmap 4's shard lock is an **unbounded spinlock**, so a guard alive across `.await` lets the worker pick up another task, hit the same shard, and spin at 100% CPU in a non-async loop — it never returns to the scheduler to poll the future that would release the guard. Both workers wedge, every task stops, the watchdog force-exits at 90 s, and the hole in `trades` is never replayed. Copy the fields out, drop the guard, then await (`DeadFlush` in `token_cache.rs`). `scripts/check-async-guards.sh` gates it. |
