...
pub fn trade_share(burst: &WindowState, reference: &WindowState, now: Ts) -> f64 {
    let denom = reference.trade_count(now);
    if denom > 0.0 {
        burst.trade_count(now) / denom * 100.0
    } else {
        f64::NAN
    }
}
...
mod tests {
    ...
    fn tape(widths: [f64; 2], at: &[f64]) -> (WindowState, WindowState) {
        let mut a = WindowState::new(widths[0]);
        let mut b = WindowState::new(widths[1]);
        for (i, &t) in at.iter().enumerate() {
            a.on_trade(Side::Buy, 1.0, ts(t), i as u64);
            b.on_trade(Side::Buy, 1.0, ts(t), i as u64);
        }
        (a, b)
    }
    ...
    fn a_burst_and_an_even_drip_carry_the_same_volume_and_read_differently() {
        ...
        assert_eq!(trade_share(&burst_a, &ref_a, now), 50.0);
        // Only the 60.0 print is inside 3s.
        assert_eq!(trade_share(&burst_b, &ref_b, now), 10.0);
    }
    ...
    fn an_empty_reference_window_is_nan_not_zero() {
        ...
        assert!(trade_share(&burst, &reference, ts(201.0)).is_nan());
    }
    ...
    fn a_token_younger_than_the_burst_window_reads_one_hundred() {
        ...
        assert_eq!(trade_share(&burst, &reference, ts(1.0)), 100.0);
    }
    ...
    fn the_share_decays_on_a_tick_that_outruns_the_burst_window() {
        ...
        assert!((trade_share(&burst, &reference, ts(30.0)) - 100.0 / 3.0).abs() < 1e-9);
        // 15s of silence later the burst is empty; the reference still holds all three.
        assert_eq!(trade_share(&burst, &reference, ts(45.0)), 0.0);
    }
}
