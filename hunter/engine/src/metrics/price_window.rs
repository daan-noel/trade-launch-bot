...
pub struct PriceWindowState {
    ...
    /// The cursor is already in the window unit - see [`WindowSpec::pos`].
    max_deque: VecDeque<(f64, i64)>,
    /// Monotonic **increasing** by price (front = window min); newest at back.
    min_deque: VecDeque<(f64, i64)>,
}

impl PriceWindowState {
    pub fn new(spec: WindowSpec) -> Self {
        Self { spec, max_deque: VecDeque::new(), min_deque: VecDeque::new() }
    }
    ...
    #[inline]
    fn holds(&self, pos: i64, now_pos: i64) -> bool {
        let (lo, hi) = self.spec.bounds(now_pos);
        pos >= lo && pos <= hi
    }

    /// The span this tracker covers.
    ...
    pub fn on_trade(&mut self, price: f64, pos: i64, now_pos: i64) {
        if !price.is_finite() {
            return;
        }
        ...
        self.max_deque.push_back((price, pos));
        ...
        self.min_deque.push_back((price, pos));
        self.evict(now_pos);
    }
    ...
    fn win_high(&self, now_pos: i64) -> Option<f64> {
        self.max_deque
            .iter()
            .filter(|&&(_, at)| self.holds(at, now_pos))
            .map(|&(p, _)| p)
            .max_by(|a, b| a.partial_cmp(b).unwrap())
    }

    /// Lowest price in `[now − w, now]` (`None` when empty).
    fn win_low(&self, now_pos: i64) -> Option<f64> {
        self.min_deque
            .iter()
            .filter(|&&(_, at)| self.holds(at, now_pos))
            .map(|&(p, _)| p)
    ...
    fn current_price(&self, now_pos: i64) -> Option<f64> {
        self.max_deque
            .iter()
            .filter(|&&(_, at)| self.holds(at, now_pos))
            .max_by_key(|&&(_, at)| at)
    ...
    pub fn trail(&self, now_pos: i64) -> f64 {
        match (self.win_high(now_pos), self.current_price(now_pos)) {
    ...
    pub fn rise(&self, now_pos: i64) -> f64 {
        match (self.win_low(now_pos), self.current_price(now_pos)) {
...
mod tests {
    ...
    fn nan_before_first_trade() {
        let s = PriceWindowState::new(WindowSpec::secs(30.0));
        let now = p(0.0);
    ...
    fn trail_is_percent_below_rolling_high() {
        let mut s = PriceWindowState::new(WindowSpec::secs(30.0));
        s.on_trade(10.0, p(0.0), p(0.0));
        assert_eq!(s.trail(p(0.0)), 0.0); // at the high
        s.on_trade(12.0, p(1.0), p(1.0)); // new high, still at it
        assert_eq!(s.trail(p(1.0)), 0.0);
        s.on_trade(9.0, p(2.0), p(2.0)); // 25% below the 12 high
        assert!((s.trail(p(2.0)) - 25.0).abs() < 1e-9);
    }
    ...
    fn rise_is_percent_above_rolling_low() {
        let mut s = PriceWindowState::new(WindowSpec::secs(30.0));
        s.on_trade(10.0, p(0.0), p(0.0));
        assert_eq!(s.rise(p(0.0)), 0.0); // at the low
        s.on_trade(8.0, p(1.0), p(1.0)); // new low, still at it
        assert_eq!(s.rise(p(1.0)), 0.0);
        s.on_trade(10.0, p(2.0), p(2.0)); // 25% above the 8 low
        assert!((s.rise(p(2.0)) - 25.0).abs() < 1e-9);
    }

    #[test]
    fn rolling_high_expires_off_the_front() {
        let mut s = PriceWindowState::new(WindowSpec::secs(10.0));
        s.on_trade(20.0, p(0.0), p(0.0)); // high at t=0
        s.on_trade(10.0, p(5.0), p(5.0)); // 50% below the 20 high
        assert!((s.trail(p(5.0)) - 50.0).abs() < 1e-9);
        ...
        s.on_trade(10.0, p(11.0), p(11.0));
        assert_eq!(s.trail(p(11.0)), 0.0);
    }
    ...
    fn non_finite_price_ignored() {
        let mut s = PriceWindowState::new(WindowSpec::secs(30.0));
        s.on_trade(10.0, p(0.0), p(0.0));
        s.on_trade(f64::NAN, p(1.0), p(1.0));
        s.on_trade(f64::INFINITY, p(2.0), p(2.0));
        assert_eq!(s.trail(p(2.0)), 0.0);
        assert_eq!(s.rise(p(2.0)), 0.0);
    }

    #[test]
    fn regressed_block_time_excludes_future_dated_entries() {
        let mut s = PriceWindowState::new(WindowSpec::secs(30.0));
        s.on_trade(10.0, p(10.0), p(10.0));
        s.on_trade(8.0, p(12.0), p(12.0));
        // At now=4 every stored print is future-dated — window is empty.
        s.evict(p(4.0));
        assert!(s.trail(p(4.0)).is_nan());
        // At now=12 both prints lie in the window.
        assert!((s.trail(p(12.0)) - 20.0).abs() < 1e-9); // 8 is 20% below the 10 high
        assert_eq!(s.rise(p(12.0)), 0.0); // 8 is the low, current is 8
    }

    #[test]
    fn slot_order_regressed_block_time_does_not_phantom_trail_spike() {
        let mut s = PriceWindowState::new(WindowSpec::secs(5.0));
        // Low slot seen first in slot order but carrying a later block_time.
        s.on_trade(0.000082, p(54.0), p(54.0));
        // High stream with an earlier block_time seen second.
        s.on_trade(0.000121, p(51.0), p(51.0));
        // At now=51 the t=54 print must not inflate trail against the t=51 print.
        assert!(s.trail(p(51.0)) < 30.0);
    }
    ...
    fn deques_match_naive_scan_over_a_series() {
        ...
        let mut s = PriceWindowState::new(WindowSpec::secs(10_000.0));
        for (i, &px) in prices.iter().enumerate() {
            let now = p(i as f64);
            s.on_trade(px, now, now);
            let hi = prices[..=i].iter().cloned().fold(f64::MIN, f64::max);
            let lo = prices[..=i].iter().cloned().fold(f64::MAX, f64::min);
            let expect_trail = (hi - px) / hi * 100.0;
            let expect_rise = (px - lo) / lo * 100.0;
    ...
    fn windowed_extrema_match_naive_in_window_scan() {
        ...
        let mut s = PriceWindowState::new(WindowSpec::secs(window));
        for (i, &px) in prices.iter().enumerate() {
            ...
            assert!((s.trail(at) - (hi - px) / hi * 100.0).abs() < 1e-9, "trail@{i}");
            assert!((s.rise(at) - (px - lo) / lo * 100.0).abs() < 1e-9, "rise@{i}");
        }
    }
}
