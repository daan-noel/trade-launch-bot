...
| File | Responsibility |
| --- | --- |
| `wallet_pool.rs` | Pool lifecycle primitives. `generate_wallets` (fresh ed25519 → envelope-encrypt → insert `generated`, server-side, caller never sees keys); `poll_balances_once` (`getMultipleAccounts` batched 100/call → `record_balance` → promote `generated`/`funding` → `funded` at `MIN_FUNDED_LAMPORTS=0.001 SOL`; also refreshes treasury); `sweep_reservations_once` (reservation + funding TTL sweep, 15m); `refresh_all_balances` (operator on-demand full refresh); `fresh_cached_balance` (15s cache reuse). The claim/mark-used transitions themselves are SQL on `ManagedWalletRepo`. |
| `wallet_lifecycle.rs` | **Unified lifecycle tick** `spawn_wallet_lifecycle` — folds four loops into one shared-wake task: (1) balance poll+promote (adaptive 5s active / 30s idle), (2) reservation+funding TTL sweep (30s), (3) warm-pool top-up funding (60s, only if funding configured), (4) hourly dust sweep. |
| `wallet_funding.rs` | **Treasury → pool funding** (inverse of dust sweep). `FundingStrategy` seam (Tier-1 `DirectJittered`: one jittered transfer per wallet, timing de-corr; Tier-2 multi-hop stubbed). `fund_once` / `fund_for_launch` (JIT to the template requirement) / background pass. Plain `solana-client` transfer (no Jito — no landing urgency). Guarded by `FundingConfig` reserve floor + per-interval cap + `FUNDING_LOCK` (serializes all passes) + `FUND_DRY_RUN`. |
| `wallet_sweep.rs` | Operator-triggered reclaim passes (Wallet Pool buttons): `sweep_used_and_retired` + `consolidate_all`, sharing the plain-transfer SSOT + a token-account closer. |
| `dust_sweep.rs` | Hourly `used`-wallet dust sweep home to treasury, then retire. Open-position guard (`TokenPositionRepo`) protects a wallet still holding tokens. Plain transfer; `SWEEP_MIN_LAMPORTS=0.0001 SOL`. |
