...
| `funding_plan.rs` | **JIT-funding SSOT** — `dev_launch_required_lamports` (= `min_dev_launch_lamports` + dev-buy spend) is the SAME figure the pre-launch gate enforces, so funder target and gate can't drift. `leg_required_lamports` = one co-buy's buy (SOL-in, fee inside) + base-token ATA rent (by token program) + WSOL ATA rent (v2 encodings) + signature fee + the worst persona priority fee + tip (`plan_pipeline::plan_personas`, the set the gate disguises with) + the payer's rent-exempt minimum; `FUNDING_HEADROOM_LAMPORTS` pads the dev target only. |
...
| `plan.rs` | `build_plan` — turn a request into a previewable plan (pure DB reads, no chain). Sell sizing: `pct_of_holdings` (integer basis points, 100% = the whole balance) or `fixed_base` (a token amount clamped to the balance). |
