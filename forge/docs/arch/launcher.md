...
| `wallet_lifecycle.rs` | `spawn_wallet_lifecycle` — a single **DB-only safety timer** (60s): the reservation/funding TTL sweep (`sweep_reservations_once`), which releases wallets stranded `reserved`/`funding` by a crashed/aborted launch. The former automatic balance poll, warm-pool funder, and hourly dust sweep were removed (zero idle Helius RPC). No RPC, no SOL movement. |
| `wallet_funding.rs` | **Treasury → pool funding** (inverse of dust sweep), **operator-triggered only** ("Fund pool" button → `fund_once`). `FundingStrategy` seam (Tier-1 `DirectJittered`: one jittered transfer per wallet; Tier-2 multi-hop stubbed). Each send is **confirmed** and the wallet **promoted `funding` → `funded` in-place** (a post-fund `getMultipleAccounts` read-back), so one click leaves the pool claimable — no background poll. Plain `solana-client` transfer (no Jito). Guarded by `FundingConfig` reserve floor + per-interval cap + `FUNDING_LOCK` + `FUND_DRY_RUN`. (The autonomous background funder + JIT `fund_for_launch` were removed.) |
...
| `dust_sweep.rs` | `used`-wallet dust sweep home to treasury, then retire — **operator-triggered** via the "Sweep & retire" button (`sweep_used_wallets`; the former hourly automatic pass was removed). Open-position guard (`TokenPositionRepo`) protects a wallet still holding tokens. Plain transfer; `SWEEP_MIN_LAMPORTS=0.0001 SOL`. |
