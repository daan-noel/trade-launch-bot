...
| `map.rs` | Pure `IngestEvent` → platform-core row mappers (no DB/network, unit-testable). `trade_to_row` / `token_created_to_row` / `raw_tx_to_row`. Takes amounts from EXACT raw-`u64` lamport fields (`sol_lamports`, `virtual_sol_lamports`) — no f64 round-trip; decodes base58 sig → BYTEA; keeps `payer_net_lamports` only when the tx payer is the trade's wallet (`own_payer_net`). `PUMP_TOKEN_DECIMALS = 6`. |
