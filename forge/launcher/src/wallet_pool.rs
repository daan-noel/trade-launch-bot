...
use chrono::Utc;
use platform_core::models::{ManagedWallet, NewManagedWallet, WalletRole};
...
/// otherwise falls through to a live read.
...
/// reuse the balance a recent "Refresh balances" / "Fund pool" pass wrote instead
/// of issuing a second identical `get_balance`. Pre-send correctness is unchanged:
/// a fresh cache value is a confirmed-commitment read from seconds ago, and the
...
/// One reservation + funding TTL sweep, driven by the `wallet_lifecycle` timer.
/// Cheap and RPC-free — bounded to the small `reserved`/`funding` sets by the
/// partial indexes in migration `0004`.
...
///
/// Operator-triggered burst (the "Refresh balances" button) — batched
/// `getMultipleAccounts` (100 pubkeys/call), one cache write per wallet. With the
/// automatic balance poll removed, this (and the post-fund promote in
/// `wallet_funding`) is the ONLY thing that refreshes cached balances and promotes
/// `generated`/`funding` → `funded`. Returns the freshly-stamped rows (balance +
/// `balance_checked_at`), optionally scoped to one role, in list order.
