...
//!
//! NOTE: a small subset of the protocol IDs / unit conversions and the CU
//! tuning constants are intentionally duplicated in `trading_core::config::
//! constants` so `trading_core`/`lab` can read them without depending on this
//! crate (which pulls the full trading/RPC executor stack). Keep the two copies
//! in sync if a shared value ever changes (program IDs effectively never do).
