//! `backend-local` — the analysis-box binary. Composition root for the local
...
use backend_local::{api, state, storage, sweep};

use backend_core::config::{self, constants};
use backend_core::{models, services};
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "backend_local=info,sqlx=error".into()),
        )
        .init();
