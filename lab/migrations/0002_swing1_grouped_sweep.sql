-- ============================================================================
-- swing_1 grouped param-sweep storage (kill→volume swing-phase strategy).
--
-- Lab-only, same lifecycle/ownership as `0001_grouped_sweep.sql` (written only by
-- `lab`, tracked in `_lab_migrations`). A four-table set matching the registry's
-- `GroupedSweepTables` (`swing_1_grouped_sweep_{runs,groups,results,combos}`),
-- identical in shape to the tpsl1/tpsl2 sets EXCEPT the `_results` table carries
-- one extra exit-reason count: `n_exit_next_kill`.
--
-- `n_exit_next_kill` is swing1's symmetric "the dev started another kill" exit
-- (kernel `ExitCode::NextKill`). tpsl1/tpsl2 can never produce it, but the sweep
-- repo is strategy-blind and binds the same column set for every strategy, so we
-- also `ADD COLUMN` it (DEFAULT 0) to their existing `_results` tables here. This
-- is a separate migration (not an edit to `0001`) because the lab migration
-- ledger rejects any change to an already-applied migration's body.
-- ============================================================================

-- ---------------------------------------------------------------------------
-- Backfill the shared exit-reason column onto the pre-existing strategy tables.
-- ---------------------------------------------------------------------------
ALTER TABLE tpsl2_grouped_sweep_results
    ADD COLUMN IF NOT EXISTS n_exit_next_kill INTEGER NOT NULL DEFAULT 0;
