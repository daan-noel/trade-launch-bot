...
/// columns), the live runtime counters (always 0 on the local box), and a derived
/// lifecycle. The shape matches the deploy edge's `rule_to_json`.
...
/// — the frontend `useRulePositions` hook consumes both identically.
