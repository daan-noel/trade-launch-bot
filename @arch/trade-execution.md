...
| `swap_retry.rs` | **SSOT** `classify_swap_revert(custom, route, direction) -> SwapRetryDecision` — maps a landed revert's on-chain code to a retry decision (slippage retry, 2006 creator/coin_creator refresh, 6024 cashback refresh, 6005 reroute). Shared by this crate's own `confirm=true` in-call heal (`sell.rs`/`buy.rs`/`amm.rs`) and `live`'s feed-confirmed bot loop, which imports it directly instead of keeping its own copy |
