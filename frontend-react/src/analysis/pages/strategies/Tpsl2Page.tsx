...
import { useBackgroundJobActions } from '@analysis/context/BackgroundJobsContext';
import { apiErrorMessage, useGetTokensByMintsQuery } from 'store/apiSlice';
import { useSellTokenMutation } from '@deploy/store/deployEndpoints';
...
} from '@analysis/store/strategyResultCache';
import type { AppDispatch } from '@analysis/store';
import type {
  PaperResultResponse,
  PaperRunResponse,
...
function positionToSimResult(p: RulePositionRecord): SimulatedTokenResult {
  ...
  // entry_token_amount is a TOKEN count; SOL invested = entry_price Ã— tokens, so
  // PnL in SOL = (entry_price Ã— entry_token_amount) Ã— pnl%.
...
export function Tpsl2Page() {
  ...
  // positionColumns/simColumns are now referentially-stable module constants â€”
  ...
  const handleSave = async (unlocked: LockGroupState) => {
    ...
    // ceiling must sit above the floor â€” the backend rejects max <= min with 400.
