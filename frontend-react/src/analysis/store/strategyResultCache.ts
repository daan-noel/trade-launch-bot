import { analysisApi, type StrategyRuleArg } from './analysisEndpoints';
import { store, type AppDispatch } from '@analysis/store';
...
export function fetchMatchedCached(
  ...
  return run(dispatch(analysisApi.endpoints.getStrategyMatched.initiate(arg, opts(force))));
}
...
export function fetchSimulateCached(
  ...
  if (!force) {
    const cached = analysisApi.endpoints.getStrategySimulateResult.select(arg)(store.getState());
  ...
  return (async () => {
    ...
    return run(
      dispatch(analysisApi.endpoints.getStrategySimulateResult.initiate(arg, { forceRefetch: true })),
    );
  })();
}
...
export function fetchPaperResultCached(
  ...
  return run(dispatch(analysisApi.endpoints.getStrategyPaperResult.initiate(arg, opts(force))));
}
