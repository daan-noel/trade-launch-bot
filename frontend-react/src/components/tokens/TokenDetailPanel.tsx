...
function buildInstructionList(instr: unknown) {
  const valueToString = (v: unknown) => (typeof v === 'string' ? v : JSON.stringify(v));
  let items: string[] = [];

  if (Array.isArray(instr)) {
    items = instr.map(valueToString);
  } else if (instr && typeof instr === 'object' && 'instructions' in instr) {
    const arr = (instr as { instructions?: unknown[] }).instructions;
    if (Array.isArray(arr)) items = arr.map(valueToString);
  }
  ...
  return (
    <div className="flex max-h-[240px] flex-col gap-0.5 overflow-y-auto">
      {items.map((text, i) => (
        <div
          key={i}
          className="rounded border-l-2 border-white/8 bg-white/2 px-1 py-0.5 text-left text-[10px] leading-snug text-text whitespace-pre-wrap break-words"
        >
          {text}
        </div>
      ))}
...
export function TokenDetailPanel({ detail, loading, error }: TokenDetailPanelProps) {
  ...
  const copyLabels = async () => {
    try {
      await navigator.clipboard.writeText(JSON.stringify(detail.instruction_labels));
      setCopied(true);
  ...
  return (
    <section className="flex flex-col gap-2">
      ...
      <div className="flex items-start gap-0">
        ...
        <div className="flex w-[185px] shrink-0 flex-col gap-1 overflow-y-auto max-h-[300px]">
          <div className="flex items-center justify-between text-[9px] font-bold uppercase tracking-widest text-text-dim">
            ...
            <button
              ...
              onClick={copyLabels}
              className={cn(
                'inline-flex size-[34px] items-center justify-center rounded-lg border border-white/12 bg-white/4 text-text transition hover:bg-white/8',
                copied && 'border-primary/30 bg-primary/12 text-primary',
              )}
              title={copied ? 'Copied!' : 'Copy labels'}
            >
              {copied ? '✓' : '⎘'}
            </button>
          </div>
          {buildInstructionList(detail.instruction_labels)}
        </div>
      </div>
    </section>
  );
