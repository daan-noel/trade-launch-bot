...
import { store, type AppDispatch } from './index';
...
export function fetchSimulateCached(
  ...
  if (!force) {
    const cached = apiSlice.endpoints.getStrategySimulateResult.select(arg)(store.getState());
  ...
  return (async () => {
    ...
    return run(
      dispatch(apiSlice.endpoints.getStrategySimulateResult.initiate(arg, { forceRefetch: true })),
    );
  })();
}
