...
// `buy_amount`, `tolerance_pct`, `trade_mode`, …). Form state, the clipboard blob,
...
export interface ParamField {
   ...
   *  e.g. `p_exit_take_profit`, `buy_amount`, `tolerance_pct`, `trade_mode`. */
  column: string;
   ...
   *  otherwise blank→null/0. Only TP/SL and `buy_amount`. */
  required: boolean;
