...
import { walletColumns } from '@deploy/components/wallet/walletColumns';
import { CashbackCard } from '@deploy/components/wallet/CashbackCard';
...
} from '@deploy/store/deployEndpoints';
import type { AppDispatch } from '@deploy/store';
  ...
  const confirmTrade = useCallback(
    async (mint: string, prevAmount: number | undefined, label: string) => {
      for (let attempt = 0; attempt < 5; attempt += 1) {
        ...
        const sub = dispatch(
          deployApi.endpoints.getWalletHolding.initiate(mint, { forceRefetch: true }),
        );
        try {
          const holding = await sub.unwrap();
          if ((holding?.amount ?? undefined) !== prevAmount) {
            dispatch(
              deployApi.util.updateQueryData('getWalletHoldings', undefined, (draft) => {
      ...
      dispatch(deployApi.util.invalidateTags(['WalletHoldings']));
      setActionSuccess(
