...
import { LiveModeControl } from '@deploy/components/LiveModeControl';
import { usePositionNotifications } from '@deploy/hooks/usePositionNotifications';
import { deployNav } from './nav';
...
const SyncTokenPage = lazy(() => import('@deploy/pages/tokens/SyncTokenPage').then((m) => ({ default: m.SyncTokenPage })));
const TransactionsPage = lazy(() => import('@deploy/pages/transactions/TransactionsPage').then((m) => ({ default: m.TransactionsPage })));
const MyWalletPage = lazy(() => import('@deploy/pages/profiles/MyWalletPage').then((m) => ({ default: m.MyWalletPage })));
...
/** Mounts the global position-notification toasts (deploy-only). */
