...
use backend_core::config::constants::{resolve_buy_slippage_bps, resolve_sell_slippage_bps};
use pump_trader::constants::LAMPORTS_PER_SOL;
use backend_core::models::ingest::SseEvent;
use backend_core::models::Position;
...
impl Tpsl1StrategyService {
    pub fn spawn_background_tasks(&self) {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                // buy task was lost to a restart/resume (never entered, exited, or
                // deleted otherwise). Without the real pass these 0-entry Holding
                // rows linger forever — gated out of every exit path AND pinning
                // their dead token in `token_cache` (`is_held`).
    ...
    pub async fn on_token_created(&self, mint: &str, cache: &TokenCache) {
        ...
        for rule_id in rule_ids {
            ...
            let token_program_id = position
                .token_program_id
                .clone()
                .unwrap_or_else(|| backend_core::config::constants::TOKEN_PROGRAM_ID.to_string());
            let paper_repo = self.paper_repo.clone();
            ...
            tokio::spawn(async move {
                ...
                if is_real {
                    // Claim the entry slot for the buy's lifetime — the buy-side
                    // twin of the sell's ExitGuard. While held, the buy-recovery
                    // reaper skips this position (the buy is genuinely in flight);
                    // the RAII guard frees the slot when this task ends OR panics.
    ...
    async fn try_close_manually_sold(
        &self,
        position: std::sync::Arc<backend_core::models::Position>,
        mint: String,
        sell_price: f64,
