...
use backend_core::models::{Position, Tpsl2Rule};
...
pub(crate) async fn reconcile_externally_cleared_mint(
    ...
    let last_sell = trade_repo
        .find_latest_by_wallet_mint_type(&wallet, mint, backend_core::models::trade::TradeType::Sell)
        .await
        .ok()
        .flatten();
    let mut closed = 0;
                ...
                if balance <= super::PARTIAL_FILL_THRESHOLD as f64 {
                    if let Ok(Some(last_sell)) = trade_repo
                        .find_latest_by_wallet_mint_type(
                            &wallet,
                            &mint,
                            backend_core::models::trade::TradeType::Sell,
                        )
                        .await
                    {
                        info!(
...
async fn sell_until_balance_cleared(
    ...
    if amount == 0 {
        let legs = trade_repo
            .sum_legs_by_signatures(&wallet, &mint, &sell_sigs, backend_core::models::trade::TradeType::Sell)
            .await
            .ok()
            .flatten();
...
mod tests {
    ...
    use backend_core::config::constants::TOKEN_PROGRAM_ID;
    ...
    async fn setup(
        ...
        sqlx::query(
            "INSERT INTO tpsl2_strategy_rules (id, rule_name, buy_amount, p_exit_take_profit, p_exit_stop_loss) \
    ...
    async fn cleanup(pool: &PgPool, mint: &str, position_id: Uuid) {
        ...
        let _ = sqlx::query(
            "DELETE FROM tpsl2_strategy_rules WHERE id = \
             (SELECT rule_id FROM tpsl2_real_positions WHERE id = $1)",
        )
        .bind(position_id)
        .execute(pool)
        .await;
