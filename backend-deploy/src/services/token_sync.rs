...
mod amm_verification {
    ...
    use super::*;
    use backend_core::config::constants::{PUMP_FUN_PROGRAM_ID, PUMP_SWAP_PROGRAM_ID, WSOL_MINT};
        ...
        for s in sigs {
            ...
            if trades.is_empty() {
                ...
                let derived_event = logs.iter().find_map(|log| {
                    ...
                    let disc = &bytes[..8];
                    let is_swap = disc == backend_core::config::constants::PUMP_SWAP_BUY_EVENT_DISCRIMINATOR
                        || disc == backend_core::config::constants::PUMP_SWAP_SELL_EVENT_DISCRIMINATOR;
                ...
                match derived_event {
                    ...
                    Some((_, quote_sol)) if backend_core::models::trade::Trade::is_dust(quote_sol) => {
    ...
    fn first_swap_event_pool(logs: &[&str]) -> Option<String> {
        for log in logs {
            ...
            let disc = &bytes[..8];
            if disc == backend_core::config::constants::PUMP_SWAP_BUY_EVENT_DISCRIMINATOR
                || disc == backend_core::config::constants::PUMP_SWAP_SELL_EVENT_DISCRIMINATOR
            {
