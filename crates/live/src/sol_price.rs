//! SOL/USD poller — updates `quote_assets.usd_rate` for SOL (id=1). USDC stays
//! seeded at 1.0; USD remains derived in views only (ADR D4).
...
const SOL_QUOTE_ASSET_ID: i16 = 1;
const POLL_INTERVAL_SECS: u64 = 60;
const COINGECKO_URL: &str =
    "https://api.coingecko.com/api/v3/simple/price?ids=solana&vs_currencies=usd";
const WSOL_MINT: &str = "So11111111111111111111111111111111111111112";

/// Fetch SOL/USD, preferring CoinGecko with a Jupiter fallback.
pub async fn fetch_latest_sol_price() -> anyhow::Result<f64> {
    match fetch_coingecko().await {
        ...
        Err(cg_err) => {
            ...
            fetch_jupiter().await.map_err(|jup_err| {
...
async fn fetch_jupiter() -> anyhow::Result<f64> {
    let url = format!("https://api.jup.ag/price/v3?ids={WSOL_MINT}");
    let client = reqwest::Client::builder()
        .timeout(Duration::from_secs(10))
        .build()?;
    ...
    v.get(WSOL_MINT)
        ...
        .and_then(|u| u.as_f64())
        .ok_or_else(|| anyhow::anyhow!("Jupiter response missing WSOL usdPrice"))
}

/// Background task: poll every 60s and write `quote_assets.usd_rate` for SOL.
pub async fn run_poller(pool: Arc<PgPool>) {
    info!("SOL/USD poller: starting (every {POLL_INTERVAL_SECS}s)");

    loop {
        match fetch_latest_sol_price().await {
            Ok(price) => {
                match QuoteAssetRepo::set_usd_rate(&pool, SOL_QUOTE_ASSET_ID, price).await {
