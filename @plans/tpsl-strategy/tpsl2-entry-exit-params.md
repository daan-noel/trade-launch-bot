...
**Fill convention (sim/backtest/sweep):** when the ladder first fires on trade T in slot S, the fill window is **slot S (trades after T only) + the next observed slot after S**, provided the next slot is within `MAX_FILL_WAIT_SLOTS` (≈ 1 s). The recorded exit price is the **lowest-priced trade in the window** — worst case for the seller.

**Empty window (analysis vs live split):** if that window holds no trade, the two callers of the walk diverge:
- **Analysis path** (`find_trade_driven_exit` — backtest / grouped sweep / swing1-detect): fills at the **firing trade itself** (a market exit modelling the bot's own sell). Without this, a token whose price provably crossed TP/SL right at a sparse, gappy stretch was mislabeled **`Open`**. This bites **migrated tokens** hardest: the crossing lands on the curve blow-off just before migration, where the next curve trade is `> MAX_FILL_WAIT_SLOTS` away *and* the post-migration AMM fills aren't in the local corpus (the lake/PG mirror holds only the token's `curve` legs). tpsl2's `run_exit_walk` **locks onto the first firing slot**, so the fallback is the fill for that slot. **tpsl1 and swing1 share this fix** but their walks **rescan every later firing first** (they only reach the fallback when the *whole* TP region is un-fillable), and it's plumbed through a private `exit_walk(…, market_fill)` with `find_trade_driven_exit` = `true` and a strict live sibling = `false`.
- **Live paper poll** (tpsl2/swing1 `find_trade_driven_exit_with_slot`, tpsl1 `find_trade_driven_exit_live`): keeps the strict window and returns `None` — an empty window means the real fill hasn't indexed yet, so the poll waits it out. On timeout the poll (`paper.rs`) books the exit at the **trigger price** as terminal `ExitFailed` (the fill was never confirmed, so NOT a clean win — but no longer zeroed to a −100% total loss for tpsl2). Real trading (`real.rs`) is unchanged: a sell that never clears the balance is a genuine on-chain `ExitFailed`.

**Analysis death-close (`ExitReason::Dead`).** The market-fill fallback above only rescues a position whose ladder *fired* — a token that simply **dies by going silent** never crosses a trade-driven rule at all, so the analysis walk returns `None` and the token was still booked `Open` (marked to a stale, often high last price → overstated PnL). The analysis `find_trade_driven_exit` in all three strategies appends one final `.or_else(|| strategies::death::find_death_point(trades, entry_time, Utc::now()))`. `find_death_point` walks the post-entry trades once (newest real-reserve snapshot + newest meaningful trade time), asks the shared `token_cache::is_dead_verdict` (liquidity `< DEAD_MAX_LIQUIDITY_SOL` **and** no `>= DEAD_MEANINGFUL_TRADE_SOL` trade for `>= DEAD_QUIET_SECS`), and on a dead verdict closes at the **death point** = last meaningful post-entry trade (dust tail doesn't reset the clock or set the price). That verdict fn is the exact one live's `TokenState::is_dead` calls, so sim and live can never disagree on *which* tokens are dead. Analysis-only: live's strict poll never calls it (it closes silent tokens through the 1s clock sweep). Rolled up as **closed** (`ExitCode::Dead=9`, `RunMetrics.n_exit_dead`, grouped-sweep `n_exit_dead` column — `lab/migrations/0004`).
