...
/// Strategy rule for **tpsl_sniper_1** — the token-creation-filter strategy.
/// Fully independent of tpsl2: it carries only the common token-filter and
/// exit-ladder fields, with no scalp-continuation gates (tpsl1's
/// `tpsl1_strategy_rules` table has no such columns). The matching repo,
/// handler and runner are likewise tpsl1-only.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct Tpsl1Rule {
    pub id: Uuid,
