...
use super::super::{ExitGuard, Tpsl1RuntimeCache};
use trading_core::models::ingest::SseEvent;
use trading_core::models::{Position, PositionStatus, Tpsl1Rule};
use crate::state::token_cache::{CachedTrade, TokenCache};
use trading_core::storage::repositories::tpsl1_paper_trading_repo::Tpsl1PaperTradingRepo;
            ...
            if let Some(fill) = super::super::entry::find_entry_fill_in_trades(&trades, 5) {
                ...
                if let Ok(Some(prev)) = paper_repo.find_by_id(position_id).await {
                    ...
                    if let Ok(current) = paper_repo
                        .update_entry(position_id, &entry_tx, buy_amount, fill.price, fill.block_time)
                        .await
                    {
                ...
                info!(
                    ...
                    position_id, fill.price, entry_tx
                );
                recorded = true;
                break;
            }
