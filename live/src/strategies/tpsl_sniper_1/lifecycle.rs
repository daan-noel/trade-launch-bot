...
use trading_core::models::{Position, Tpsl1Rule};
...
pub async fn stop_and_close_rule(
    ...
    let now = Utc::now();
    // Sell slippage for the force-close batch: None = no floor (always fills).
    let slippage_bps =
        trading_core::config::constants::resolve_sell_slippage_bps(app_state.settings().sell_slippage_bps, None);
    ...
    for position in positions {
        ...
        let Some(guard) = app_state.tpsl1_cache.try_begin_exit(position_id) else {
            continue;
        };
