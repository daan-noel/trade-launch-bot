...
| Param Sweep | `strategies/SweepPage.tsx` | per-strategy (`<SweepPage strategy=… />`); `getSweepRuns`/`getSweepResults`; run picker + ranked param-pair DataTable (client-side sort/filter/paginate); columns built by `buildSweepColumns` (param cols derived from run params) |
