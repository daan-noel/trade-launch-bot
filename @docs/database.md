...
- `raw_transactions` *(PARTITIONED)* — id, signature, slot, block_time, raw_data(JSONB), received_at, source(`grpc`/`rpc`). PK (received_at,id). RANGE-by-week on received_at, ~9wk retention +2wk forward; fns `ensure_raw_partition`/`drop_raw_partition`. No UNIQUE (dupes tolerated). Both the live gRPC pipeline (`source='grpc'`) and token_sync backfill (`source='rpc'`) persist the full Helius blob — kept for later analysis/replay.
