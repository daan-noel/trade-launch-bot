...
- `tpsl2_grouped_sweep_runs` — strategy_id, source(`db`), method, created_after/before, curve_only, grouping_spec(JSONB GroupField[]), axes_spec(JSONB resolved axes), min_tokens, token_count, group_count, combo_count, corpus_hash, created_at. Idx: (created_at DESC). (The sweep synthesizes its base rule in-process; no rule_id is stored.)
