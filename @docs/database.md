...
- `raw_transactions` *(PARTITIONED)* — id, signature, slot, block_time, raw_data(JSONB), received_at, source(`live`/`sync`). PK (received_at,id). RANGE-by-week on received_at, ~9wk retention +2wk forward; fns `ensure_raw_partition`/`drop_raw_partition`. No UNIQUE (dupes tolerated). `source='live'` = real-time LaserStream ingest; `source='sync'` = token_sync backfill (both "Fetch All" via gTFA and "Fetch New" via LaserStream replay). Both persist the full Helius blob — kept for later analysis/replay.
