...
- `tpsl2_grouped_sweep_runs` — strategy_id, rule_id, source(`db`), method, created_after/before, curve_only, grouping_spec(JSONB GroupField[]), axes_spec(JSONB resolved axes), min_tokens, token_count, group_count, combo_count, corpus_hash, created_at. Idx: (created_at DESC).
...
- `tpsl2_grouped_sweep_results` — run_id + group_id (both FK CASCADE), combo_id, params(JSONB), + the flat per-combo metric set: counts (n_fired/open/closed), profitability (win_rate, total_pnl_sol, expectancy_sol, profit_factor, mean/median/p90/best/worst pnl%, std_pnl_pct), `score` (robust rank μ−z·σ/√n over closed trades, NULL when n_closed<2 — the drill-in's default sort), holding (avg/median secs), exit-reason mix (`n_exit_{take_profit,stop_loss,trailing,stall,time,liquidity,cohort,open}` — per-reason trade **counts**, `n_`-prefixed to distinguish from the `exit_*` *threshold* knobs inside `params`). Served per-group whole, sorted/filtered client-side. Idx: (group_id, combo_id).
- `tpsl1_grouped_sweep_{runs,groups,results}` (migration `0004_tpsl1_grouped_sweep`) — **identical shape** to the tpsl2 triple above (same generic repo writes both). TPSL1 sweeps the exit ladder only, so `params`/`best_params`/`axes_spec` carry fewer keys and `n_exit_cohort` is always 0 (column kept for schema parity with the generic INSERT).
