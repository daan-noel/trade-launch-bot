...
| `maintenance.rs` | `run_partition_maintenance()` | Every 6h: ensure today+2 future **daily** partitions and drop > `KEEP_DAYS` (30) for **both** `raw_transactions` (on `received_at`) and `trades` (on `block_time`). Partition span switched weekly→daily in migration 0003 (non-destructive: old weekly partitions age out, new days seed as overlaps clear). |
