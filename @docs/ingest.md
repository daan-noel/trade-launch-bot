...
| `adapter.rs` | `build_raw_blob()`, `compiled_ix()`, `inner_ix()`, `token_balances()` | Protobuf → Helius JSON blob (the persisted `raw_data`). `build_raw_blob` runs off-thread in the DbWriter for live (`source='grpc'`) txs and inline in token_sync for backfilled (`source='rpc'`) txs, so blobs are byte-consistent across sources. Account-key order: static ++ loaded_writable ++ loaded_readonly. |
...
| `grpc/mod.rs` | **The protobuf-native decoder.** `decode_protobuf(&SubscribeUpdateTransaction, received_at)` reads the protobuf directly (no `Value`, no per-ix base58 round-trip); `PbIx` (borrowed program-id + `accounts`/`data` bytes) + protobuf-native key/ix/label/account helpers + `decode_amm_live_pb` (AMM, pool resolved via the shared index). Null-data raw-tx carrier (events embed but never read `raw_tx`; the blob is built by `build_raw_blob`). `#[cfg(test)]` unit tests decode synthetic txs and assert the expected events. |
