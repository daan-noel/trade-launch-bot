...
| `backtest.rs` | `BacktestTokenResult`, `run_backtest` | replay using the same `exit::find_trade_driven_exit` as live; candidates from the in-memory `token_cache` (not a full `tokens` scan). Trade history is fetched in **batched chunks** (`trade_repo.find_by_mints_all`, `BACKTEST_FETCH_CHUNK` mints/query) with only `BACKTEST_FETCH_CONCURRENCY`≈3 chunk queries in flight, so a running sim holds ~3 of the ~20 shared PgPool conns instead of starving live ingest (was one query/token at concurrency 16); per-token resolution is synchronous over the in-hand history. Emits real progress via `SimProgress` — one `tick()` per resolved candidate (any outcome), broadcast as throttled `SseEvent::SimulationProgress` so the dashboard bar is determinate, not a fake trickle. **tpsl2 only:** `target_*` = the `find_scalp_entry` trigger; `entry_*` = the worst-case fill from `find_worst_case_paper_entry` (mirrors paper); exit/PnL/ATH are driven off the *fill* price, not the trigger; a token with no priced fill (`entry_fill.price <= 0`) is dropped, as paper deletes an un-filled position. `target_*` are `Option` on the struct (`None` only for legacy paper rows mapped by `paper_position_to_sim_result`). |
