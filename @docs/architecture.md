...
**Supervision & liveness:** the final `tokio::select!` treats *any* long-lived task resolving (return/panic/abort, via `task_fault`) as fatal → `main` exits non-zero so a supervisor restarts the process. That catches task *death* but **not** a task deadlocked on an `.await` (e.g. ingest parked on a full `update_tx.send()` behind a wedged DbWriter) — those used to freeze silently. The `state/ingest_health.rs` liveness watchdog (a dedicated **OS thread**, stamped by the LaserStream client on each forwarded tx) covers that hole: stale heartbeat past `INGEST_STALL_TIMEOUT_SECS` while live → `process::exit(1)`. See `@docs/ingest.md` for the full chain.
