...
- `app_state.rs` — `AppState` (db — the **dedicated API pool** from `postgres::connect_api_pool`, isolated from the hot-path `db` so an ingest/strategy write storm can't exhaust the connections the dashboard needs; helius urls, all caches, watch channels, `sse_tx` + `sse_frame_tx`, `pool_index`, `pools_changed`, `trade_signals`, `sync_gate`, `backtest_trade_cache`, `trader`, `pump_program_id`). Also exposes repo accessors (`token_repo()`, `trade_repo()`, `settings_repo()`, `tpsl{1,2}_{rule,position,paper}_repo()`, `wallet{,_profile,_tag}_repo()`, `analysis_repo()`) so handlers call `state.x_repo()` instead of `XRepo::new(state.db.clone())`. `SyncGate` bounds `/api/token/sync` (max 4 concurrent, dedup by mint → 409 on collision).
...
- `settings.rs` — `Settings::from_env()`: Helius (api key, RPC, Sender URLs, LaserStream), wallet key, nonce accounts, DB url + **two-pool sizing** (`DB_MAX_CONNECTIONS` hot path vs `DB_API_MAX_CONNECTIONS` API; `DB_ACQUIRE_TIMEOUT_SECS`, `DB_IDLE_TX_TIMEOUT_SECS`), server host/port/workers/CORS/`API_AUTH_TOKEN`.
