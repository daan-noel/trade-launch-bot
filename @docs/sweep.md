...
- `api/handlers/strategies/grouped_sweep.rs` — generic handler set:
  - `POST /api/strategies/sweeps` (`start_grouped_sweep`; body
    ...
    SSE), and on every exit path its `Gate` releases the single-flight
    gate, resets `sweep_progress`, and emits the terminal `SweepFinished`.
