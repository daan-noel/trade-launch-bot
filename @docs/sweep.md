...
- **Sweep memory guard** (`check_sweep_memory` in `registry.rs`): the combo cap
  alone doesn't bound RAM — the fold holds `vec![ComboAgg; combos]` per active
  fold and the small-group phase runs up to `threads` concurrently, so peak ≈
  `threads × combos × sizeof(ComboAgg)`. Before any allocation the run is rejected
  if that exceeds `SWEEP_MEMORY_BUDGET_MB` (default 1024), turning a process-killing
  OOM `abort()` into a clean error. Pairs with the slimmed `ComboAgg` (~0.6 KB).
