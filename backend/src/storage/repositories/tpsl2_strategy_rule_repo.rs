...
struct Tpsl2StrategyRuleDbRow {
    ...
    p_token_cu_limit: Option<i64>,
    p_token_cu_price: Option<i64>,
    ...
    p_token_ix_labels: Json<serde_json::Value>,
    trade_mode: String,
    buy_amount: f64,
    ...
    p_entry_higher_low_secs: Option<i64>,
    p_entry_max_cohort_held: Option<f64>,
    p_entry_min_liquidity_sol: Option<f64>,
    p_entry_min_organic_liq: Option<f64>,
    p_exit_cohort_ratio: Option<f64>,
    tolerance_pct: f64,
    is_active: bool,
...
impl From<Tpsl2StrategyRuleDbRow> for Tpsl2Rule {
    fn from(r: Tpsl2StrategyRuleDbRow) -> Self {
        Self {
            id: r.id,
            rule_name: r.rule_name,
            p_token_initial_buy_sol: r.p_token_initial_buy_sol,
            p_token_cu_limit: r.p_token_cu_limit.map(|v| v as u64),
            p_token_cu_price: r.p_token_cu_price.map(|v| v as u64),
            ...
            p_token_ix_labels: r.p_token_ix_labels.0,
            ...
            p_entry_higher_low_secs: r.p_entry_higher_low_secs.map(|v| v as u64),
...
impl Tpsl2StrategyRuleRepo {
    ...
    pub async fn insert(&self, rule: &Tpsl2Rule) -> anyhow::Result<()> {
        sqlx::query(
            r#"
        ...
        .bind(rule.id)
        .bind(&rule.rule_name)
        .bind(rule.p_token_initial_buy_sol)
        .bind(rule.p_token_cu_limit.map(|v| v as i64))
        .bind(rule.p_token_cu_price.map(|v| v as i64))
        .bind(rule.p_token_max_sol_cost)
        .bind(rule.p_token_spendable_sol_in)
        .bind(rule.p_max_concurrent_tokens.map(|v| v as i64))
        .bind(rule.p_max_total_tokens.map(|v| v as i64))
        .bind(Json(&rule.p_token_ix_labels))
        .bind(rule.trade_mode.clone())
        .bind(rule.buy_amount)
        .bind(rule.p_exit_take_profit)
        .bind(rule.p_exit_stop_loss)
        .bind(rule.tolerance_pct)
        .bind(rule.is_active)
        .bind(rule.created_at)
        .bind(rule.updated_at)
        .bind(rule.p_exit_trailing_stop_pct)
        .bind(rule.p_exit_time_stop_secs.map(|v| v as i64))
        .bind(rule.p_exit_stall_secs.map(|v| v as i64))
        .bind(rule.p_exit_liquidity_drop_pct)
        .bind(rule.p_entry_min_age_secs.map(|v| v as i64))
        .bind(rule.p_entry_min_alive_sol)
        .bind(rule.p_entry_min_organic_sol)
        .bind(rule.p_entry_pullback_pct)
        .bind(rule.p_entry_higher_low_secs.map(|v| v as i64))
        .bind(rule.p_entry_max_cohort_held)
    ...
    pub async fn find_all(&self) -> anyhow::Result<Vec<Tpsl2Rule>> {
        ...
        Ok(rows.into_iter().map(Tpsl2Rule::from).collect())
    }

    /// Get a specific rule by ID.
    pub async fn find_by_id(&self, rule_id: Uuid) -> anyhow::Result<Option<Tpsl2Rule>> {
        ...
        Ok(row.map(Tpsl2Rule::from))
    }

    /// Update an existing TPSL rule.
    pub async fn update(&self, rule: &Tpsl2Rule) -> anyhow::Result<()> {
        sqlx::query(
            r#"
