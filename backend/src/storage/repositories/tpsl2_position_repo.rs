impl Tpsl2PositionRepo {
    ...
    pub async fn revert_exit_pending(&self, position_id: Uuid) -> anyhow::Result<()> {
        sqlx::query(
            r#"
            UPDATE tpsl2_positions SET status = 'Holding', updated_at = $2 WHERE id = $1
            "#,
        )
        .bind(position_id)
        .bind(Utc::now())
    ...
    pub async fn delete_position(&self, position_id: Uuid) -> anyhow::Result<()> {
        sqlx::query("DELETE FROM tpsl2_positions WHERE id = $1")
            .bind(position_id)
            .execute(&self.pool)
            .await?;
        Ok(())
