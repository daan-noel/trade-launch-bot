...
//   cargo test --bin backend tpsl2_position_repo:: -- --ignored --nocapture
// ---------------------------------------------------------------------------
#[cfg(test)]
mod tests {
    use super::*;
    ...
    async fn seed_position(
        repo: &Tpsl2PositionRepo,
        mint: &str,
        status: PositionStatus,
        age: Duration,
        entered: bool,
    ) -> Uuid {
        let mut pos = Position::new(mint.to_string(), unique("W"), "TPSL2".to_string(), Uuid::new_v4());
        pos.status = status;
        if entered {
    ...
    async fn cleanup(pool: &PgPool, mint: &str) {
        let _ = sqlx::query(
            "DELETE FROM tpsl2_strategy_rules WHERE id IN \
             (SELECT rule_id FROM tpsl2_real_positions WHERE mint = $1)",
        )
        .bind(mint)
        .execute(pool)
        .await;
        let _ = sqlx::query("DELETE FROM tpsl2_real_positions WHERE mint = $1")
            .bind(mint)
            .execute(pool)
            .await;
    }
