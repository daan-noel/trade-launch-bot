...
impl TryFrom<PaperPositionDbRow> for Position {
    ...
    fn try_from(r: PaperPositionDbRow) -> Result<Self, Self::Error> {
        ...
        Ok(Self {
            ...
            // Paper sends no buys — always empty (real-only durable marker).
...
impl Tpsl2PaperTradingRepo {
    ...
    pub async fn clear_runs(&self, rule_id: Uuid) -> anyhow::Result<u64> {
        let result = sqlx::query("DELETE FROM tpsl2_paper_test_run WHERE rule_id = $1")
            .bind(rule_id)
            .execute(&self.pool)
            .await?;
    ...
    pub async fn find_by_run(
        ...
        let rows = sqlx::query_as::<_, PaperPositionDbRow>(&format!(
            "SELECT {POSITION_COLS} FROM tpsl2_paper_positions WHERE run_id = $1 ORDER BY created_at ASC LIMIT $2 OFFSET $3"
        ))
        .bind(run_id)
        .bind(limit)
        .bind(offset)
    ...
    pub async fn find_all_holding(&self) -> anyhow::Result<Vec<Position>> {
        let rows = sqlx::query_as::<_, PaperPositionDbRow>(&format!(
            "SELECT {POSITION_COLS} FROM tpsl2_paper_positions WHERE status IN ('Holding','Arming','BuySubmitted') ORDER BY created_at DESC"
        ))
        .fetch_all(&self.pool)
        .await?;
        rows.into_iter().map(Position::try_from).collect()
    ...
    pub async fn count_by_run_all(&self) -> anyhow::Result<std::collections::HashMap<Uuid, i64>> {
        let rows: Vec<(Uuid, i64)> = sqlx::query_as(
            "SELECT run_id, COUNT(*) FROM tpsl2_paper_positions WHERE entry_price IS NOT NULL GROUP BY run_id",
        )
        .fetch_all(&self.pool)
        .await?;
