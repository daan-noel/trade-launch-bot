//! The `Value` decode path: decode a Helius `jsonParsed` `params.result` (RPC
//! `getTransaction` for token_sync/replay, or — pre-Tier-B — a live update the
//! adapter synthesised into a `Value`). The protobuf-native live path is the
//! sibling [`super::grpc`]; both share the byte/log leaves at [`super`] root
//! (`create`, `trade`, `instructions`) and are guarded equal by the parity tests.
...
use super::instructions::{determine_instruction_type, InstructionKind};
use super::trade::{
...
use instructions::{
    build_instruction_labels, collect_instruction_kinds, instruction_data_bytes,
...
use trade::decode_trade_events_from_inner_ixs;
...
impl HeliusDecoder {
    ...
    pub fn decode_result(&self, result: Value) -> DecodeOutput {
        ...
        if has_create {
            if let Some(create_ix) = pump_ixs.iter().find(|ix| is_pump_create_ix(ix)) {
                ...
                // Decode the raw instruction-data bytes once per pump ix (base58
                // → bytes) and feed the source-agnostic `decode_create` plain
                // slices, matching the protobuf path's byte inputs.
                let create_data = instruction_data_bytes(create_ix).unwrap_or_default();
                let pump_ix_datas_owned: Vec<Vec<u8>> = pump_ixs
                    .iter()
                    .filter_map(|ix| instruction_data_bytes(ix))
                    .collect();
                let pump_ix_datas: Vec<&[u8]> =
                    pump_ix_datas_owned.iter().map(|v| v.as_slice()).collect();
    ...
    /// (pump_amm) swap. Unlike [`Self::decode_result`], this path keys off the
    ...
    /// LaserStream subscription. Mirrors [`Self::decode_pump_swap_result`] but, since
