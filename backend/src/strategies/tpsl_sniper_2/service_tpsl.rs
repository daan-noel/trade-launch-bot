            ...
            if let Some(rule) = handler.get_rule(rule_id) {
                ...
                if rule.trade_mode == "paper" {
                    ...
                    tokio::spawn(async move {
                        ...
                        for _ in 0..Tpsl2StrategyService::BUY_POLL_MAX_ATTEMPTS {
                            sleep(Duration::from_millis(
                                Tpsl2StrategyService::BUY_POLL_INTERVAL_MS,
                            ))
                            .await;
...
/// `Tpsl2StrategyService::BUY_*` constants; tests shrink it so the give-up and
...
impl BuyRetryCfg {
    fn production() -> Self {
        Self {
            max_attempts: Tpsl2StrategyService::BUY_MAX_ATTEMPTS,
            backoff_ms: 500,
            poll_attempts: Tpsl2StrategyService::BUY_POLL_MAX_ATTEMPTS,
            poll_interval: Duration::from_millis(Tpsl2StrategyService::BUY_POLL_INTERVAL_MS),
        }
    }
}
    ...
    if let Ok(trades) = trade_repo.find_by_mint(&mint, 20).await {
        if let Some(last_sell) = trades.into_iter().find(|t| {
            ...
            let remaining = trade_repo
                ...
                .unwrap_or(0.0)
                .max(0.0);
            if remaining <= Tpsl2StrategyService::PARTIAL_FILL_THRESHOLD {
...
async fn sell_with_retries(
    ...
    let max_attempts = Tpsl2StrategyService::SELL_MAX_ATTEMPTS;
    ...
    while attempt < max_attempts && amount > 0 {
        ...
        match sell_result {
            Ok(true) => {
                ...
                while poll_attempts < Tpsl2StrategyService::SELL_POLL_MAX_ATTEMPTS {
                    poll_attempts += 1;
                    match trade_repo
                        .net_token_amount_by_wallet_and_mint(&wallet, &mint)
                        .await
                    ...
                    sleep(Duration::from_millis(Tpsl2StrategyService::SELL_POLL_INTERVAL_MS)).await;
                }

                if poll_attempts >= Tpsl2StrategyService::SELL_POLL_MAX_ATTEMPTS {
                    warn!(
                        mint = %mint,
