...
use crate::models::{PaperRun, PaperRunStatus, Position, PositionStatus, Tpsl2StrategyRule};
...
pub struct Tpsl2RuntimeCache {
    active_rules: Arc<RwLock<Arc<Vec<Tpsl2StrategyRule>>>>,
    rules_by_id: Arc<RwLock<HashMap<Uuid, Tpsl2StrategyRule>>>,
...
impl Tpsl2RuntimeCache {
    ...
    fn set_rules(&self, rules: Vec<Tpsl2StrategyRule>) {
    ...
    pub fn active_rules(&self) -> Arc<Vec<Tpsl2StrategyRule>> {
        self.active_rules
            .read()
            .map(|r| r.clone())
            .unwrap_or_default()
    ...
    pub fn rule_by_id(&self, rule_id: Uuid) -> Option<Tpsl2StrategyRule> {
        self.rules_by_id
            .read()
            .ok()
